...
- Source: carried forward from ST-088 explicitly rather than closed with it — `.github/planning/story-board.md`, ST-088's *"Carried forward, deliberately not closed here"* bullet, item (1), which also states that this work **needs its own story and a minted `ST-NNN`**. Named as the single largest gap in the *select* half of the recommendation at [docs/investigations/ST-084-awcp-host-spike-findings.md](../../docs/investigations/ST-084-awcp-host-spike-findings.md) §18.9 (`:1958`) and §18.11, and **directed rather than concluded** by [ADR-016](../../docs/design/adr/ADR-016-awcp-consolidation-host-topology.md) §1(b) at the 2026-08-26 sign-off. The PO supplied a concrete topology to score at sign-off (`docs/investigations/Product stack.txt`)
...
- Acceptance criteria:
  - [ ] **Re-verify the ground before citing it.** Every anchor this story rests on moved in the sign-off change and will move again: ADR-016 §1's criterion 6 sat at `:54` before 2026-08-26 and at `:85` after; findings §18.11 sat at `:2048` on `main` and `:2065` after. Cite ADR-016, the board and findings §18.10–§18.11 by **§-anchor**, not line; keep line cites only where they were verified stable across both trees (`ports.ts:109` / `:162`; findings anchors at or before `:1958`; `awcp-spec-evaluation.md:179` / `:185`). Findings §12a is the standing worked example of Stage 1 claims moving underneath a later reader
  ...
  - [ ] **Scored against the same six criteria, reproduced exactly, that A, B and C each received** — **domain fit, security model, code maturity, migration effort, operational simplicity, retirement path** (`docs/investigations/awcp-spec-evaluation.md:179`; the frame ADR-016 §1's retained **A-vs-C** table uses, and the frame `prism-ground-truth-inventory.md` §4 used to score B). **Three candidates, not four** — Candidate D was rejected by PO direction (`awcp-spec-evaluation.md:186`) and never scored; findings §18.9 and §18.11 both say "A, B, and C". Any claim that D was scored is unsupported
...
- Docs: [docs/investigations/ST-084-awcp-host-spike-findings.md](../../docs/investigations/ST-084-awcp-host-spike-findings.md) §18.3 (`:1764` — the ports seam and its read-side scope gap), §18.4 (`:1806` — the recommended topology, and why it is **not** Candidate C), §18.5 (`:1871` — domain ownership inventory), §18.6 (the one-chokepoint caveat), §18.8 (`:1927` — bounded extraction roadmap A–E, **sketch only, not started**), §18.9 (`:1958` — missing evidence), §18.11 (verdict and the PO-supplied target stack — **§-anchor only; this section moved in the sign-off change**), §13.5 (`:1123` — the withdrawn cross-topology day-count), §19.2 (repo-rescan, carried forward with an owner); [docs/design/adr/ADR-016-awcp-consolidation-host-topology.md](../../docs/design/adr/ADR-016-awcp-consolidation-host-topology.md) §1 (the two-part decision, the interface requirement, the tenancy ground, and the retained **A-vs-C** scoring table — **§-anchor only**); [docs/investigations/awcp-spec-evaluation.md](../../docs/investigations/awcp-spec-evaluation.md) §7 (`:179` the six criteria; `:185` C's donor-retirement condition; `:186` D rejected by PO direction, never scored); [docs/investigations/prism-ground-truth-inventory.md](../../docs/investigations/prism-ground-truth-inventory.md) §4 (Candidate B scored on the same six criteria) and §5 (`:144` — the wiki as *"Frozen, not donated"*, and `:8`'s fact/interpretation caveat on that disposition); [docs/investigations/awcp-external-evidence-import-2026-08.md](../../docs/investigations/awcp-external-evidence-import-2026-08.md) `:63`, `:84` (agent-radio evidence and the exact scope of its STOP); [docs/design/adr/ADR-013-platform-product-definitions.md](../../docs/design/adr/ADR-013-platform-product-definitions.md) `:94` (platform-vs-product litmus), `:96`/`:102` (product register; the Workflow/Operations row as updated at revision 1.3), `:110` ("Layering is not deployment"); [docs/investigations/awcp-strategy-baseline-2026-08.md](../../docs/investigations/awcp-strategy-baseline-2026-08.md) `:118-129` (decisions 1–3); `docs/investigations/Product stack.txt` (the PO's target stack, committed alongside the sign-off because the scoring rests on it); `server/src/workflow/ports.ts:109`, `:162`; `server/tests/workflow-boundary.test.ts`
