...
- Value: 4
- Blocked by: **the ADR-016 sign-off landing on `main`.** This story's entire premise is that sign-off. It was committed on `docs/st088-adr016-signoff` (ADR-016 → Accepted rev 1.5, ADR-013 → rev 1.3, findings §18.10 → APPLIED, ST-088 → Review) and is unblocked the moment that branch merges — until then `main` still reads `status: "proposed"`, so **verify before grounding anything on it**. `ST-100` was minted through `./story-id.sh` and its allocator line is on `main`, so the ID is usable. Deliberately **not** blocked on **ST-082** — criterion 5 is an *input* to the scoring, not a prerequisite for it. Not blocked on **ST-088** either, though note ST-088 sits in **Review, not Done**, precisely because it carried this gap forward rather than filling it
...
- Value: 3
- Blocked by: **ST-088** — baseline decision 3 puts the first new milestone at Horizon B, *after* ST-088 closes and ADR-016 leaves Proposed/Conditional. **Half-discharged 2026-08-26:** ADR-016 has left Proposed/Conditional — it is **Accepted** (rev 1.5) with Candidate A rejected — so only "ST-088 closes" remains, and ST-088 is in Review. Nothing here may be executed before that; the plan exists so the sequence is written down rather than rediscovered
