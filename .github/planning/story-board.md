...
- Acceptance criteria:
  ...
  - [ ] Full `deno test tests/` goes green — **now 222 passed / 3 failed** in CI run 28654276687 (was 219/6). Only the 3 `graph_traverse` failures remain (deferred below).
- Plan: (to be created — `docs/plans/`)
- Notes: Surfaced 2026-07-03 the first time the integration suite ever ran in CI. All 6 failures are in files **unchanged from `main`** (`git diff --name-only origin/main...HEAD -- server/` shows only the two ST-070 health files) — so they are pre-existing on `main`, not introduced by PR #21 or ST-070. Root story: the CI `integration-tests` job has been red since inception (secret gate → type error → these), so the suite's green state was never established. **Progress 2026-07-03 (commit `3c0dd1a`, PO scope "fix only the trivial ones now"):** the 3 test-isolation/fixture failures (2× duplicate-key, 1× null-`query`) are fixed and confirmed green in CI (222 passed / 3 failed, run 28654276687). The remaining 3 `graph_traverse` comment-handling failures are genuine assertion/behavior drift, **deferred** for a dedicated pass — determine whether the MATCH-only/comment-stripping guard changed behavior or the expectations are stale, and reconcile. These 3 still block PR #21's checks going fully green.
