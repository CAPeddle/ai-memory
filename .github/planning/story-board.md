...
- Value: 5
- Blocked by: **nothing to start D0.** A3 is blocked on the two local branches landing or being lifted. **B2–B4 are blocked on the D0-4 PO gate** (an explicit ADR-016 §3 revisit). **A6 is blocked on ST-088 closing** — that is the milestone boundary at which the new structure becomes authoritative
...
- Acceptance criteria:
  ...
  - [ ] **D0 — whether `ST-NNN` is the human-facing id for AWCP-native work is recorded as an OPEN PO decision** with both options and their consequences. It is not inferred from the development ledger
  - [ ] **D0 — the storage question goes through ADR-016 §3**, per `awcp-spec-evaluation.md:177` (*"not to be silently resolved by the first migration author"*). Returns permit-or-defer for the migration
  ...
  - [ ] **B — the web UI is the primary surface** and renders a WorkItem with its packets, sessions and attention. `awcp status` exists as a secondary diagnostic consuming the *same* read model
  ...
  - [ ] **B — a healthy captured session produces zero attention items, and its Red/Green Control shows an abandoned one producing exactly one.** The zero alone is not evidence
