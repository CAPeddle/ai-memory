...
- Acceptance criteria:
  ...
  - [x] **Criterion 6 — remote Ubuntu execution node** (authenticated registration, heartbeat, checkpoint, repo-state; offline spool + idempotent replay; disconnection/duplicate/invalid-auth experiments). Discharged for every element the criterion names (auth, heartbeat, checkpoint, spool, replay, experiments 4-6) — z2 enrolled and exercised for real via PR #50 (`b32b6ab`, 2026-08-18); findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §16.5 disposition table. Repo-rescan is explicitly out of criterion 6's own wording (recorded as a U3 scope gap, not a shortfall here) — not built, doesn't qualify the discharge. Corrected 2026-08-25: this box and the note below had gone stale, still showing Wave 6 as outstanding four days after it actually merged
