...
- Acceptance criteria:
  ...
  - [ ] **Criterion 7 — final extraction viability and the final ADR-016 recommendation.** Extraction-viability judgment complete and a full recommendation drafted 2026-08-26 — findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §18: criteria 1-4 and 6 met; **criterion 5 stays unproven** (U1 priced the enforcement surface at 64+ hours but `scope.tags` is still enforced in zero paths, and the read-side port carries no `PolicyScope` — priced is not enforced, and its checkbox above correctly stays unticked); criterion 7 (does ai-memory's reuse justify Candidate A's domain-fit cost) answered **no**, recommendation is reject Candidate A / select Candidate C (standalone AWCP, ai-memory as optional context provider via an adapter contract derived from `ports.ts`, with `PolicyScope` threaded through its read side as a precondition). Box stays unchecked because the recommendation is **held for PO sign-off, not yet applied to ADR-016** — its live `status` and Decision text are untouched, per explicit PO direction this session. §18.10 carries the exact proposed replacement text
