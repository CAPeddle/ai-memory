...
### ST-091: Retarget the .NET stack from net8.0 to net10.0 (LTS, before .NET 8 EOL)
- Type: infrastructure
...
- Acceptance criteria:
  ...
  - [ ] `global.json` moved to the `10.0.1xx` band (note the current `rollForward: latestPatch` is a tight pin — it accepts only `8.0.1xx`, not even the `8.0.4xx` feature band)
  - [ ] `Directory.Build.props`: `TargetFramework` → `net10.0`, `LangVersion` raised from 12
