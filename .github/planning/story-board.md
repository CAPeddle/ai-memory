...
- Acceptance criteria:
  ...
  - [ ] **D0 — the storage gate is a narrow override of ADR-016 §1, not §3 compliance.** §3 is explicitly *"not a host decision"*, so it cannot lift §1's bar. D0-4 asks for a dated override covering migration `005` **only** — every later AWCP migration returns for its own decision — plus the §3 revisit `awcp-spec-evaluation.md:177` requires *(PO decision 2026-08-24)*
