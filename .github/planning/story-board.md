...
- Acceptance criteria:
  ...
  - [x] **Criterion 6 — remote Ubuntu execution node** (authenticated registration, heartbeat, checkpoint; offline spool + idempotent replay; disconnection/duplicate/invalid-auth experiments). Discharged for every element the criterion names (auth, heartbeat, checkpoint, spool, replay, experiments 4-6) — z2 enrolled and exercised for real via PR #50 (`b32b6ab`, 2026-08-18); findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §16.5 disposition table. **Corrected 2026-08-26: `repo-state` struck from this clause.** It never appeared in the criterion itself — `docs/design/adr/ADR-016-awcp-consolidation-host-topology.md:54` reads *"authenticated remote event ingestion with spooled replay"* — and this paraphrase had silently widened it, leaving the board and the ADR asserting different gates. The ADR's wording is ratified as governing (PO, 2026-08-26); findings **§19** records the ratification and discharges `03-CONTEXT.md:250-252`'s instruction that Phase 4 settle this. **Repo-rescan itself is not retired** — it stays a real U3 capability gap, carried forward in §19.2 with its design direction (recast the push half onto the existing checkpoint payload; the pull half is separable) and with hub→node SSH recorded as rejected against the spike plan's own no-remote-shell constraint. Corrected 2026-08-25: this box and the note below had gone stale, still showing Wave 6 as outstanding four days after it actually merged
  - [ ] **Criterion 7 — final extraction viability and the final ADR-016 recommendation.** Extraction-viability judgment complete and a full recommendation drafted 2026-08-26 — findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §18: criteria 1-4 and 6 met; **criterion 5 stays unproven** (U1 priced the enforcement surface at 64+ hours but `scope.tags` is still enforced in zero paths, and the read-side port carries no `PolicyScope` — priced is not enforced, and its checkbox above correctly stays unticked); criterion 7 (does ai-memory's reuse justify Candidate A's domain-fit cost) answered **no**. Recommendation is a **two-part decision of unequal weight**: (a) *reject Candidate A* — settled on the spike's full evidence; (b) *direct a standalone AWCP peer service* — ai-memory kept live as an optional context provider via an adapter derived from `ports.ts`, with `PolicyScope` threaded through its read side as a precondition. **(b) is a direction, not a scored selection, and is explicitly NOT Candidate C** — C is defined by retiring both donors, which this does not do; the peer-service topology was never scored against the six criteria, and scoring it is the named next step. Box stays unchecked because the recommendation is **held for PO sign-off, not yet applied to ADR-016** — its live `status` and Decision text are untouched, per explicit PO direction this session. §18.10 carries the exact proposed replacement text
...
- **Phase 4: recommendation drafted, held for PO sign-off** 2026-08-26 (U5+U6) — matching `.planning/ROADMAP.md`'s phrasing, not "complete": U5 is fully done (`blocking` state re-verified UNPROVEN against current code, findings §17), and U6's re-evaluation and recommendation are fully drafted (findings §18: criteria 1-7 re-evaluated, a defended two-part recommendation — reject Candidate A (settled), and direct a standalone AWCP peer service (a direction, unscored, explicitly **not** Candidate C) — a domain-ownership inventory, a bounded non-big-bang extraction sketch, and the proposed ADR-016 replacement text at §18.10), but **the phase's own success criteria require ADR-016 itself to record the decision**, and that has deliberately not happened: ADR-016's `status` and Decision remain Proposed/Conditional, held for explicit PO sign-off per direction given this session — the story stays In Progress (not Review) until that sign-off lands.
