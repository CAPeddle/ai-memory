...
> Last updated: 2026-09-25 (**AWCP stories transferred to [CAPeddle/awcp](https://github.com/CAPeddle/awcp).** By PO direction AWCP owns its own work and this board keeps only work that builds ai-memory. ST-100, ST-097, ST-096, ST-094 and ST-102 moved Backlog → Archived with transfer notes (awcp #19–#23), and ST-103's requirements moved to awcp by PR #24. ST-094 and ST-102 fix code that still runs here as the extraction donor, so their fixes land in ai-memory until extraction.)
> Previously: 2026-08-28 (**ST-102 filed to Backlog — `/gsd-audit-milestone` caught a designed escalation that was never received.** Running the milestone audit *before* `/gsd-complete-milestone` was the whole point: it found no undelivered requirement and no broken cross-phase link, and exactly one dropped handoff. Phase 3's threat-register row `T-03-01-02` created the unauthenticated `/workflow` dashboard shell knowingly and said in its own text *"Escalated, not closed … Do not resolve this by silently leaving the flag on"*; it was routed to Phase 4 twice (this board `:914`, `:969`) and Phase 4's *"Carried forward"* block — written from Phase 4's own findings rather than from a sweep of its inbox — never listed it. **ST-102 now owns it.** Two further audit warnings are recorded but not filed: `run_events`/`execution_nodes` have no read surface (belongs with the peer-service extraction, ST-100), and every bare `§13` citation is ambiguous because the findings document carries two `## 13.` headings. Audit report: [`.planning/v1.0-MILESTONE-AUDIT.md`](../../.planning/v1.0-MILESTONE-AUDIT.md), status `gaps_found` — which reads on artifact absence, not on undelivered work. **A sibling from the same review is still unowned**: `last_seen_at` never advancing on ingestion. WIP slots unchanged — both still free.)
> Previously: 2026-08-27 (**ST-088 Review → Done — PR [#60](https://github.com/CAPeddle/ai-memory/pull/60) merged to `main` as `86473ac`, and the B–D milestone is now fully unblocked.** The sign-off below discharged the *host* condition on 2026-08-26; the baseline gates B–D on two conditions, and this transition closes the second. `/gsd-new-milestone` may generate B–D, subject to the three constraints recorded on ST-097's entry — most importantly that it must **not** assume ST-100's scoring outcome, since ADR-016 §1(b) is a direction, not a scored selection. The merge also carried the corpus-wide sweep of every artifact that had recorded the discharged condition (25+ files, nine review rounds) and one CI fix. **Both WIP slots are now free.** Trailer verified parsing on the squash; branch deleted, recoverable at tag `archive/st088-adr016-signoff`.)
> Previously: 2026-08-26 (**ADR-016 signed off — the AWCP host decision is taken.** The PO accepted the ST-084/ST-088 recommendation: **Candidate A rejected**, AWCP becomes a **standalone peer service** consuming ai-memory as an optional, **replaceable** context provider (the interface is the contract; MCP/A2A is an adapter detail). ADR-016 → Accepted (rev 1.5), ADR-013 → rev 1.3 (its disposition-(b) premise was falsified by the reversal). ST-088 moved In Progress → Review. Three things are carried forward as separate stories rather than closed: the **peer-service topology is still unscored** against the six criteria A/B/C each received, **criterion 5** moves to ST-082 reframed as ai-memory's own isolation obligation, and the **prism-llm-wiki knowledge migration** into ai-memory — a PO reversal of that wiki's "frozen, not donated" disposition, content only, driven from the prism side. Effort is not an input to architecture evaluation here, by standing PO direction.)
>
> Previously: 2026-08-23 (**Board hygiene: ST-092 moved Review → Done, and ST-093 lifted onto `main`.** ST-092 squash-merged as `69b50bd` (PR #52) on 2026-08-22, but its entry was still sitting under `## Review`, holding that slot against the 1-in-Review WIP limit above — moved to the top of `## Done` with the merge SHA, PR number, and (newly checked) CI result: PR #52 targeted `main` directly rather than a stacked branch, so unlike the entry's own in-flight notes, CI *did* run and all three checks (`contact-memory-tests`, `dotnet-build`, `server-integration-tests`) went green. Separately, `docs/st-093-entity-queue-isolation` (tip `f41980b`) held the *only* copy of ST-093's board entry, and that branch **must never be merged**: its merge-base with `main` is `4b05e95`, 3 commits behind `main` (`git rev-list --count origin/docs/st-093-entity-queue-isolation..origin/main`), and `git diff --numstat origin/main origin/docs/st-093-entity-queue-isolation` sums to 175 additions / 7,860 deletions — merging it would revert ST-092's node-client hardening, the disposability-stamp corrections, and everything else landed on `main` since. Its ST-093 entry was lifted verbatim into `## Backlog` here instead (nothing else taken from that branch), with every `file:line` citation in it re-verified against current `main` first — all held up unchanged, since ST-092 never touched `entityWorker.ts`, `index.ts`, or either entity-worker test file.)
>
...
## Backlog
### ST-101: Migrate `prism-llm-wiki`'s knowledge content into ai-memory (reversal of "frozen, not donated")
- Type: migration / content (cross-repo — driven from `prism-llm-wiki`)
- Source: PO decision 2026-08-26, taken at ADR-016 sign-off. Recorded as a reversal at `docs/design/adr/ADR-013-platform-product-definitions.md:108` (rev 1.3), and carried forward on this board's header note and in ST-088's close-out item (4). **Those two recorded paraphrases are the source of record for this story** — the sign-off session's verbatim wording is not written down anywhere in either repo, so nothing here quotes it
- phase: 2
- Value: 3
- Blocked by: **ST-082 — the default, pending the PO's answer to open question 2 below.** The wiki's content is **corporate** (`docs/investigations/prism-ground-truth-inventory.md:32` — kubusinfo Confluence, the PRI Jira project, Azure DevOps repos), ai-memory's store is personal, and the inventory's own §6 item 1 (`:152`) names the posture as *enforced tag filters over one store* and states flatly: *"Corporate content sharing a store with personal memory is only as safe as that filter."* That filter is still not built — verified at filing: `parseContext.ts:100-101` parses `scope.tags` into `ValidatedTag[]`, but `search_thoughts` announces *"tags are not search filters in this tool"* (`server/index.ts:302`) and `list_thoughts`' query neither filters on nor selects `tags` (`server/index.ts:635`). Importing 60 pages of corporate synthesis before ST-082 lands is exactly the hazard ST-082 was promoted to a Must to prevent. **ST-099 is a sequencing dependency on the *run*, not a blocker on *capture*** — see the Notes for the mechanism and the AC that carries it
- Touches: (ai-memory side only — the migration script itself lives in `prism-llm-wiki`) `docs/investigations/prism-ground-truth-inventory.md` §5/§6 (supersession note on the reversed disposition). **Nothing else, unless open question 3 is answered "(a)"** — in which case the capture-surface work is a *separate platform story this one blocks on*, and it, not this story, touches `server/index.ts` (`capture_thought`, registered at `:488`, block running to ~`:609`), `server/db/007_*.sql` **plus** `server/db/schema.sql:39` (the repo keeps numbered migrations `001`–`006` alongside `schema.sql`; changing the `source` CHECK in only one of the two drifts fresh-created databases from migrated ones), `shared/tagGrammar.ts` and `docs/design/adr/ADR-012-tags-replace-binary-profile.md` (`:91`, reserved tag vocabulary)
- Acceptance criteria:
  - [ ] **The reversal is recorded at its source, as a reversal.** `docs/investigations/prism-ground-truth-inventory.md` §5 (`:144`) still reads *"**Frozen, not donated**"* with the qualifier *"The 60 pages freeze at current content; the review is event-triggered on a page being wanted and unavailable"*, and §6 item 2 (`:153`) repeats *"the wiki half is **frozen rather than retired** pending an event-triggered review."* Both gain a supersession note pointing at ADR-013 rev 1.3 (`:108`) and this story, carrying the reason **as ADR-013 records it** and inventing nothing beyond it. **Precision worth keeping:** the document's own fact/interpretation boundary (`:8`) already classes *"the wiki is frozen, not donated"* as an **implemented proposal, not PO-ratified** — so 2026-08-26 is the first actual PO ruling on this content, and it reverses an executed proposal rather than an accepted decision. Say that; it is the difference between a contradiction and a ratification-with-inversion
  - [ ] **The mechanisms firewall holds, explicitly.** This story touches **content only**. Prism's three mechanisms — Confluence source lineage with SHA-256 content hashing, hash-baseline drift detection (`wiki-drift-check.ps1`), and Jira↔git evidence correlation (`traceability-harvest.mjs`) — are **unaffected**: ADR-013 §2 assigns them to the **Workflow/Operations Memory** row, which `:102` now marks *"no longer hosted here"*, and `:108` says in terms that the mechanisms *"stay assigned to Workflow/Operations Memory's evidence layer"*. (ADR-016 §4's *"extract before rebuild"* at `:167-171` points the same way, but only by inference through §1's host decision — cite ADR-013 for this, not §4.) No AC in this story may import, port, or re-implement any of them, and the completed story must not leave a reader thinking ai-memory acquired drift detection
  - [ ] **Migration scope is enumerated before anything is captured, and the enumeration is allowed to shrink it.** Default position, from the inventory's own accounting (`:38`): **the 60 content pages in `knowledge/` migrate**; `index.md`, `log.md` and `state.md` do **not** (63 `.md` files total, three of them catalog/meta — `index.md` an 86-line catalog, `log.md` a 162-line append-only operations log). `_archive/` is recorded as empty. Whether the `sources/` layer holds any migratable body content is **unresolved here and must be settled prism-side**: §1.1 (`:30`) describes a three-layer architecture `sources/` (immutable, team-owned) → `knowledge/` (LLM-owned) → schema/convention files, but §1.2's store table records content only for `knowledge/`, and the inventory's own on-stored-content note (`:49`) states the registry holds **no Confluence or Jira page bodies** — ~500 bytes of derived metadata per page. Schema/convention files are convention, not knowledge, and do not migrate. Record the enumeration; do not infer it. **"N of the 60 are excluded, with reasons" is a completed enumeration, not a failed one**
  - [ ] **The minimum provenance set is met, and the *how* is a recorded PO decision — not something this story implements on its own authority.** Verified at filing, and this is the story's sharpest finding:
    - `capture_thought` **hardcodes** `source` to `'user-taught'` (`server/index.ts:545`), and the column's CHECK admits only `('user-taught', 'auto-promoted', 'observed')` (`server/db/schema.sql:39`) — there is no `imported`/`migrated` value and no parameter to set one. The `ON CONFLICT` update (`:547-558`) does not touch `source` either, so a re-import cannot correct it
    - the `metadata` jsonb column exists (`schema.sql:19`) and **is** returned by `fetch` (`server/index.ts:272`) — but it is **not caller-suppliable**: capture builds it as `{ identifiers: normalized.facets }` and nothing else (`server/index.ts:517-518`)
    - **tags are the only caller channel**, and the grammar caps what they can carry: lowercase, at most one `:` namespace separator, ≤16 tags, ≤64 chars each (`shared/tagGrammar.ts:10-12`). `source:prism-wiki` fits; a Confluence URL, a SHA-256 hash, or an ISO date does not
    - **and `list_thoughts` surfaces none of it**: it selects `id, content, memory_type, project, created_at` — no `tags`, no `source`, no `metadata` (`server/index.ts:635`). `fetch` is the one shipped read path that would show a provenance-bearing `metadata`, if anything could write one

    So a future reader **cannot currently tell** through any shipped surface that a fact came from a frozen wiki. The **minimum provenance set** is: origin repo and page path, the frozen-at anchor (`prism-llm-wiki` HEAD `5cee66b`, the commit the inventory was synced against — `prism-ground-truth-inventory.md:4`), the page's own frontmatter `sources` (its Confluence lineage), and the frozen / event-triggered-review caveat carried forward from §5. **How that set is carried is open question 3 and belongs to the PO**: **(b)** in-content provenance headers is available to this story; **(a)** a capture-surface extension is a change to a shipped platform primitive used by every product, so if the PO chooses it, it becomes its own platform story and **this story blocks on it rather than performing it**. Either answer completes this AC; "provenance cannot be made legible without a platform change, so this story blocks" is a legitimate outcome
  - [ ] **Corporate scoping applied at capture, not retrofitted — and the write-side gap is settled with ST-082, not assumed away.** Every migrated shard carries the policy scope ST-082 defines, decided jointly with ST-082: its **Controlled policy-scope field** criterion is explicit that ordinary descriptive tags are **not** the sole policy boundary, so a bare `tags:professional` is not sufficient. **Note the gap before relying on the block:** ST-082's `Touches` field names only `searchQuality.ts`, `parseContext.ts` and `server/tests/` — the *retrieval* side — and the only caller channel into `capture_thought` today is the tag list. As scoped, ST-082 could ship the filter with no way for this import to populate the field it filters on. Whether ST-082 grows a write-side channel, or this story blocks on a further one, is part of the joint decision. Negative isolation proof for this corpus specifically: migrated content is not reachable from a personal scope through any egress path
  - [ ] **The 32KB per-thought limit is checked against real page sizes before the run.** `capture_thought` rejects content over 32,768 bytes (`server/index.ts:98`, enforced at `:503-506`). The check is necessarily **prism-side** — there is no `prism-llm-wiki` checkout on this machine (verified at filing), so ai-memory cannot measure the pages. If any page exceeds it, the splitting strategy is part of this story, and splitting interacts with provenance: a split page's parts each need the same origin anchor
  - [ ] **Embedding and graph lanes reach a known state, and the run does not strand them.** See Notes for the two mechanisms, which differ. Either the import runs **after** ST-099 is fixed, or this story owns explicit recovery `UPDATE`s for **both** lanes — resetting `thoughts.embedding_attempts`, and resetting `entity_extraction_queue.status` / `attempt_count` / `retry_after`. **Re-running the import does not achieve either reset**, for the reasons in the Notes. A migrated corpus that is durably stored but permanently invisible to the vector lane and absent from the AGE graph is not a completed migration
  - [ ] **Re-runnability proven, not assumed.** `capture_thought` upserts on `ON CONFLICT (content_fingerprint)` (`server/index.ts:547`; the constraint is `UNIQUE (content_fingerprint)`, `schema.sql:48`) and **merges** tags rather than replacing them (`server/index.ts:553-558`), so a repeated import updates in place instead of duplicating. Demonstrate it: run the migration twice, show the row count unchanged and tags not duplicated. Record the two things the upsert deliberately does *not* do: it never rewrites `content` or `source`, so neither the entity-extraction trigger nor the provenance value can be corrected by re-running. Note also that the fingerprint is **global** — normalised trim/lowercase/whitespace-collapse, deliberately not scoped by project or tags (`schema.sql:45-48`) — so two wiki pages with byte-identical normalised content collapse into one shard. Confirm whether that occurs in the 60 and record it
  - [ ] **Frontmatter with no shard counterpart is dispositioned, not silently dropped.** Each page carries `tags`, `domain`, `description`, `sources`, `last-updated`, and `related-pages` with **typed relations** (`prism-ground-truth-inventory.md:38`). `tags` maps to the tag array subject to the grammar above; `domain` (one of six fixed values) has no field and must become a namespaced tag or metadata; `related-pages`' typed relations have **no shard-model counterpart at all** — the AGE graph is populated by entity extraction from content (`server/src/entityWorker.ts`), not by declared page relations. State where each goes or that it is dropped
  - [ ] **The Developer Memory question is answered by the PO and recorded, not decided in flight** — open question 1 below. Whichever way it goes, the shards land as `memory_type: 'shard'`: ADR-013 rev 1.3 (`:108`) already says *"The migrated content lands as ordinary platform shards with provenance"*, and the `wiki` tier is ADR-007 consolidation's promotion **output**, never an import target. This story must not write `memory_type: 'wiki'` rows
- Plan: (to be created — `docs/plans/`)
- Docs: `docs/design/adr/ADR-013-platform-product-definitions.md:108` (the reversal, rev 1.3) and §2 product register (`:102`, the mechanisms' assignment to Workflow/Operations Memory); `docs/investigations/prism-ground-truth-inventory.md` §1.1–§1.3, the fact/interpretation boundary (`:8`), the on-stored-content note (`:49`), §5 (`:144`), §6 items 1–2 (`:152-153`); `docs/design/adr/ADR-016-awcp-consolidation-host-topology.md` §4 (`:167-171`); `docs/design/adr/ADR-012-tags-replace-binary-profile.md:91` (reserved tag vocabulary); `docs/design/adr/ADR-007-consolidation-pipeline.md` (Developer Memory promotion policy, if the seed option is taken); board entries **ST-082** and **ST-099** (cited by story ID — board line numbers shift whenever an entry is added above them, as this PR itself demonstrated)
- Notes: **Driven from the prism side, by PO direction — so ai-memory's real question is whether its capture surface can receive a bulk curated import, and the honest answer is "durably yes, legibly no."** The write path is sound: content is stored durably before any enrichment is attempted, and `capture_thought`'s own comment is explicit that embedding is *"a deferred enrichment, not part of the contract"* (`server/index.ts:566-577`). What the surface cannot do is **record where the knowledge came from** — the four findings in the provenance AC are all live, and together they mean a shard imported today is indistinguishable from one the PO typed by hand. That gap, not the import mechanics, is the design work in this story. — **On ST-099, stated plainly: capture is not blocked; the *run* should be — and the two enrichment lanes fail and recover by different mechanisms, which the recovery step must respect.** Rows insert fine with `needs_embedding = true`, and the inline embedding failure is caught and logged without incrementing anything (`server/index.ts:594`). The **embedding** backfill is less forgiving: its catch path increments `embedding_attempts` on **any** error with no special-casing for a 401 or a disabled provider (`server/src/embeddingBackfill.ts:55-65`), and its selection clause excludes rows once `embedding_attempts >= 5` (`:33`, `MAX_ATTEMPTS = 5` at `:4`). At the default 60-second poll (`:6`) and `BATCH_SIZE = 50` (`:5`), a 60-page corpus imported while the 401 is live exhausts within minutes — the first 50 rows exhaust before rows 51-60 are ever selected — and is then permanently excluded from the sweep. The **graph** lane strands too, but *not* the same way: `entity_extraction_queue` has exponential backoff and a `retry_after` column (`server/db/graph.sql:125-126`; `backoffSeconds` = min(2^(n-1), 128) at `server/src/entityWorker.ts:19-22`, applied at `:237`) and a terminal permanent-failure status at `:229-235`, so it exhausts its own `MAX_ATTEMPTS = 5` (`:7`) faster still. **Neither lane recovers by re-running the import.** The auto-queue trigger is `AFTER INSERT OR UPDATE **OF content**` (`server/db/graph.sql:82-85`) with a further `source_fingerprint IS DISTINCT FROM` requeue guard (`:75`), and `capture_thought`'s `ON CONFLICT` never writes `content` — so an identical re-import does not fire it at all; and nothing in that upsert resets `embedding_attempts` either. Recovery on both lanes is an explicit `UPDATE`. ST-099's own diagnosis names entity extraction among the nine failures it explains. — **Value 3 rather than 4:** the knowledge is real and durable once landed, but the story's two hardest constraints (isolation, provenance legibility) are owned by ST-082 and by a platform capability gap, so its independent contribution is the corpus and the enumeration, not the controls. — **What counts as an honest completion.** The PO has decided *that* this content migrates; the enumeration and verification steps are still allowed to contradict the current expectation about **scope, sequencing and mechanism**, and doing so is a result, not a failure. Specifically: "N of the 60 pages are excluded, with reasons"; "`sources/` holds raw corporate bodies rather than references, so the corpus entering the personal store is larger than assumed — back to the PO before importing"; "provenance cannot be made legible without a platform change, so this story blocks on that story rather than working around it"; "one or more pages exceed 32KB and splitting breaks the provenance anchor, so the split strategy needs a decision first". Record any of these against the AC that produced it. — Verify the `server/index.ts`, `schema.sql`, `graph.sql` and `embeddingBackfill.ts` line references still hold when this is picked up; board entries freeze in time, and `capture_thought` is exactly the kind of surface that moves.
- Open questions (PO — presented, not decided):
  1. **Does the migrated corpus seed the Deferred Developer Memory product, or stand alone as tagged platform knowledge?** ADR-013 rev 1.3 (`:108`) leaves it to this story, so it is presented, not decided. Seeding Developer Memory gives the content a curation owner and a promotion policy (ADR-007's confidence-scored consolidation) and matches Prism's own nature — the inventory's litmus finding is that Prism *"decides almost nothing else"* but knowledge validity (§3) — but it converts ADR-013 §4(a)'s *grandfathered* consolidation worker into a live dependency of a shipped corpus, which is precisely what disposition (a)'s *"no platform capability may grow a new dependency on the wiki tier"* clause forbids in the interim. Standing alone as tagged shards keeps the platform single-tier and the import inert, but leaves 60 pages of frozen corporate synthesis with no owner deciding when they go stale — the staleness burden §5 gave as the original reason to freeze rather than maintain them.
  2. **Does the import wait for ST-082, or does the PO accept an interim isolation risk and land it sooner under a tag convention alone?** The decision settled *that* the content migrates, not *when* relative to the isolation control. This entry blocks on ST-082 as the default-safe posture; if the PO waives that, the waiver and its scope should be recorded on ST-082 as well, since it is that story's premise that would be relaxed. Related: whether ST-082 grows the write-side channel this import needs (see the corporate-scoping AC).
  3. **How is provenance carried — (a) a capture-surface extension, or (b) in-content headers?** (a) means a caller-suppliable `metadata`/provenance parameter and/or a new `source` vocabulary value with a `007_*.sql` CHECK migration plus the matching `schema.sql` edit, and surfacing `tags`/`source` through `list_thoughts` and the search lanes (`fetch` already returns `metadata`). It is a change to a shipped platform primitive used by every product, so under (a) it becomes **its own platform story and this one blocks on it**. (b) keeps the change inside this story but puts provenance in prose the BM25 lane indexes and no caller can filter on — legible to a reader, invisible to a query. The trade is queryable-but-platform-wide against self-contained-but-unfilterable; **effort is not an input here**.
  4. **Does the `sources/` layer hold any migratable body content, or is it references only?** The inventory describes three layers (§1.1, `:30`) but records content solely for `knowledge/` (§1.2, `:38`), and its on-stored-content note (`:49`) states no Confluence or Jira page bodies are stored. This is a **prism-side verification** — there is no `prism-llm-wiki` checkout on this machine — but the PO owns whether it is asked, since a `sources/` layer that does hold bodies would put raw corporate source text, not just LLM synthesis, into the personal store.
  5. **Is this a one-shot snapshot of frozen content, or a repeatable sync?** The reversal's premise is that the wiki is frozen, which argues one-shot — but §5's original disposition kept an **event-triggered review** (*"on a page being wanted and unavailable"*), and if that review can still edit a page, ai-memory holds a copy that silently diverges. Either the review is retired by this reversal (say so, and where), or the story needs a re-sync path — which the `content_fingerprint` upsert supports for edited text but not for deletions or renames, and which (see Notes) does not itself requeue entity extraction.

### ST-095: Write the CE/GSD lifecycle boundary into governance, and wire `agent_skills`
- **Scope extended 2026-09-25 (PO direction): this story now carries ai-memory's whole GSD migration.** It absorbs ST-097 Workstream A's unfinished units, A4–A7, which were split back from [CAPeddle/awcp#20](https://github.com/CAPeddle/awcp/issues/20) because they migrate *ai-memory's* own planning rather than AWCP's. A4 freezes Done/Archived as an append-only delivery ledger; A5 stages Backlog into `.planning/` as requirement candidates with a named ingestion path into `/gsd-new-milestone`; A6 hands governance over, adding the `.planning/` precedence tier, a stated active-work policy replacing the WIP limits, and an executable boundary-sync check; A7 runs the partitioned `story-board` reference sweep, which leaves the frozen record unedited. The unit definitions are in [the ST-097 plan](../../docs/plans/2026-08-23-2245-chore-st097-gsd-pivot-board-split-awcp-status-slice-plan.md) §Workstream A. A1–A3 already landed, so this entry's runtime, `claude_md_path` and `agent_skills` criteria may already be met: verify against the tree before redoing them. **Scheduled as the closing phase of milestone v1.1**, so v1.2 starts on GSD; existing `docs/plans/` and ExecPlans stay as historical record
- Type: chore
...
- Notes: Promoted from a known, previously-deferred gap (`docs/investigations/awcp-spec-evaluation.md` §4) to a **Must** now that AWCP's local source-lineage tracker (ADR-016 §4) will co-locate corporate Confluence/Jira/ADO-derived content with personal memory in the same store. Should land before or alongside that tracker's implementation, not after — verify `parseContext.ts`/`searchQuality.ts` still match this description when picked up. **Reframed 2026-08-26 (ADR-016 rev 1.5): the co-location premise above is void.** AWCP is not hosted here — Candidate A was rejected and AWCP becomes a standalone peer service, so its source-lineage tracker goes with it and will not co-locate corporate content with personal memory in this store. **The obligation is unchanged and stands on ai-memory's own merits**: `scope.tags` is enforced in zero retrieval paths regardless of where AWCP lives, which is why ADR-016 records criterion 5 as *neutral between topologies* rather than as a co-tenancy tax. Two consequences for scope: the urgency argument must be re-derived from ai-memory's own personal/corporate boundary rather than inherited from AWCP's arrival, and threading `PolicyScope` through the **read** side (`server/src/workflow/ports.ts:109` carries none while `:162` is typed), default-deny, is additionally a **precondition of the AWCP adapter contract** (findings §18.3, §18.8 Phase A) — so that half has a second, external consumer waiting on it.
...
- Touches: `server/src/workflow/` (remote-node client, policy-scope enforcement); memory-side retrieval paths (**enumerated**, not yet chosen — findings §6.1); `docs/investigations/ST-084-awcp-host-spike-findings.md` (Stage 2 report); `docs/design/adr/ADR-016-awcp-consolidation-host-topology.md` (final disposition — **only** this story may move it off Proposed/Conditional; **done 2026-08-26**, revision 1.5, Accepted); `docs/design/adr/ADR-013-platform-product-definitions.md` (revision 1.3, following the host reversal)
- Acceptance criteria:
  - [x] **Criterion 5 — policy-scope enforcement.** **Ticked 2026-08-26 for ST-088's remit only, which was to *price* it — read the annotation before reading the box.** The criterion itself is **NOT met and is not discharged**: `scope.tags` is enforced in **zero** retrieval paths and `KnowledgeSearchPort.search(query, limit)` (`server/src/workflow/ports.ts:109`) carries no `PolicyScope` at all while `PromotionInput.policyScope` (`:162`) is typed. ADR-016 §1's final gate-progress table records it as outstanding, and building it is **ST-082's**, per this entry's own Notes. The ADR-016 decision made it **neutral between topologies** — it is ai-memory's own personal/corporate isolation obligation, required whether or not AWCP ever shared the codebase, not a co-tenancy tax. Threading scope through the read side, **default-deny**, is now a *precondition* of the adapter contract rather than follow-on work (findings §18.3, §18.8 Phase A). Ticked rather than left open because a permanently unticked box on a closed story reads as abandoned work; the honest state is "ST-088 owed a number, produced it, and the obligation moved on with an owner"
  - [x] **Criterion 6 — remote Ubuntu execution node** (authenticated registration, heartbeat, checkpoint; offline spool + idempotent replay; disconnection/duplicate/invalid-auth experiments). Discharged for every element the criterion names (auth, heartbeat, checkpoint, spool, replay, experiments 4-6) — z2 enrolled and exercised for real via PR #50 (`b32b6ab`, 2026-08-18); findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §16.5 disposition table. **Corrected 2026-08-26: `repo-state` struck from this clause.** It never appeared in the criterion itself — `docs/design/adr/ADR-016-awcp-consolidation-host-topology.md:85` (§1 criterion 6; was `:54` before the 2026-08-26 sign-off shifted it) reads *"authenticated remote event ingestion with spooled replay"* — and this paraphrase had silently widened it, leaving the board and the ADR asserting different gates. The ADR's wording is ratified as governing (PO, 2026-08-26); findings **§19** records the ratification and discharges `03-CONTEXT.md:250-252`'s instruction that Phase 4 settle this. **Repo-rescan itself is not retired** — it stays a real U3 capability gap, carried forward in §19.2 with its design direction (recast the push half onto the existing checkpoint payload; the pull half is separable) and with hub→node SSH recorded as rejected against the spike plan's own no-remote-shell constraint. Corrected 2026-08-25: this box and the note below had gone stale, still showing Wave 6 as outstanding four days after it actually merged
  - [x] **Criterion 7 — final extraction viability and the final ADR-016 recommendation. Signed off by the PO and applied to ADR-016 on 2026-08-26** (revision 1.5): status Proposed/Conditional → **Accepted**, Candidate A **rejected**, a standalone AWCP peer service directed, and §1's gate progress recorded final. The PO added two things not in the reviewed draft: **interface replaceability** as a stated requirement (capability providers are named by what AWCP needs from them; the port interface is the contract and the wire protocol — in-process, MCP, A2A — is an adapter detail; ai-memory is *a* knowledge provider, not *the* knowledge layer), and a **tenancy ground** the spike did not produce (AWCP coordinates household, development-project and trading-R&D work, all of which would otherwise sit inside a memory product's schema and Postgres role). ADR-013 revision 1.3 follows, correcting a disposition-(b) premise this reversal falsified. **The peer-service topology remains unscored** and its scoring is carried forward as a separate story, not closed here. Original drafting record follows: extraction-viability judgment complete and a full recommendation drafted 2026-08-26 — findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §18: criteria 1-4 and 6 met; **criterion 5 stays unproven** (U1 priced the enforcement surface at 64+ hours but `scope.tags` is still enforced in zero paths, and the read-side port carries no `PolicyScope` — priced is not enforced, and its checkbox above correctly stays unticked *(amended 2026-08-26 — that box was subsequently ticked for ST-088's **pricing** remit only; the criterion itself is still NOT met and now belongs to ST-082. Read criterion 5's own annotation above.)*); criterion 7 (does ai-memory's reuse justify Candidate A's domain-fit cost) answered **no**. Recommendation is a **two-part decision of unequal weight**: (a) *reject Candidate A* — settled on the spike's full evidence; (b) *direct a standalone AWCP peer service* — ai-memory kept live as an optional context provider via an adapter derived from `ports.ts`, with `PolicyScope` threaded through its read side as a precondition. **(b) is a direction, not a scored selection, and is explicitly NOT Candidate C** — C is defined by retiring both donors, which this does not do; the peer-service topology was never scored against the six criteria, and scoring it is the named next step. §18.10 carries the exact proposed replacement text, and is now marked **APPLIED**
  - [x] **ADR-016's acceptance pre-condition is discharged:** the §6.1 enforcement surface is **priced** — a defended estimate of the cost of enforcing a boundary column across 15 hand-written read paths with no chokepoint, a `fetch` that accepts no context parameter, two structurally unfilterable graph tools, unscoped provider egress, and a fingerprint dedup whose `ON CONFLICT` merges tags. Added to ADR-016 §1 as a **gate** by PO decision 2026-08-03: this story may not recommend accepting Candidate A while that number does not exist. Priced in §13 (U1, 2026-08-05): 64+ hours / 8+ days total, defended per-path
  - [x] **Actual execution blocking** is proven or reported UNPROVEN with the same honesty as Stage 1 — `blocking` is still modelled state whose only implemented consequence is the attention item (`server/src/workflow/attention.ts:51`) and a dashboard tag. Re-verified 2026-08-26 against the current (post-ST-097/098) code: exactly two read sites, both observational; `completePacket` never consults decisions; findings §17
...
- Notes: **Pricing is this story's job; building is ST-082's.** They must not collide — ST-088 produces the estimate that lets the host decision be taken, ST-082 implements the enforcement once a host is settled. If ST-082 lands first the estimate becomes an actual, which is better evidence, not a conflict. **Burden of proof still sits with the spike, not the preference** — Candidate A remains the hypothesis, and the honest outcome set is still accept / accept-with-changes / recommend Candidate C. *(Settled 2026-08-26, and the burden held: the spike **rejected** its own preferred hypothesis. The outcome was a fourth one this line did not anticipate — reject A and direct a standalone peer service, which is not Candidate C because ai-memory is not retired. Left as written; it is the record of the bar the spike was held to.)* Read findings §12a before trusting any Stage 1 claim: two of the three §6 concerns had already moved by the time Stage 1 was reviewed, in opposite directions, and a third story could move them again.
...
- **Phase 3 complete** 2026-08-18. Waves 1–5 (PR #49 → `47cd90b`): node client with bounded crash-safe spool and replay, terminal-vs-deferred failure states, SAFE-01 empty-diff regression gate, SAFE-02 corpus integrity. **Wave 6 (PR #50 → `b32b6ab`, same day) also complete** — z2 enrolled through the real Phase 2 path, all four EVENT-0x requirements discharged against the real node, findings §16. `.planning/ROADMAP.md` and this entry both still showed Wave 6 as outstanding as of 2026-08-25 — corrected here; z2 remains enrolled and any suite run against the dev database de-enrols it (use `mcp-test`/`db-test`).
- **Phase 4 complete 2026-08-26** (U5+U6). U5: `blocking` state re-verified UNPROVEN against current code (findings §17). U6: criteria 1–7 re-evaluated, a defended two-part recommendation, a domain-ownership inventory, a bounded non-big-bang extraction sketch, and the proposed ADR-016 text at §18.10. Drafted, reviewed over four rounds, and **squash-merged as `1dc850d`** — *"docs(investigations): close ST-088 with a standalone-AWCP recommendation (#59)"*, CI green on all four checks, `Story` trailer verified parsing post-merge per CLAUDE.md § Merge strategy.
- **ADR-016 signed off and applied 2026-08-26** — the act Phase 4's success criteria 2 and 4 were waiting on, and the reason this entry was held In Progress rather than moved to Review. ADR-016 revision 1.5: **Accepted**, Candidate A **rejected**, standalone AWCP peer service directed, final gate-progress table recorded. ADR-013 revision 1.3 follows it. **Moved In Progress → Review 2026-08-26**, pending that commit's own PR.
- **Carried forward, deliberately not closed here:** (1) the **peer-service topology is unscored** against the six criteria A/B/C each received — findings §18.9 calls it the largest gap in the *select* half, and ADR-016 §1(b) directs the scoring rather than concluding it; filed as **ST-100** in Backlog. (2) **Repo-rescan** stays a real U3 capability gap with its design direction recorded (findings §19.2). (3) **Criterion 5** moves to **ST-082**, reframed as ai-memory's own obligation rather than a co-tenancy tax. (4) **prism-llm-wiki knowledge migration** into ai-memory — a PO decision of 2026-08-26 that reverses the wiki's "frozen, not donated" disposition for content only, driven from the `prism-llm-wiki` side through ai-memory's capture tooling; filed as **ST-101** in Backlog, unrelated to ST-088's scope.
- **Review → Done 2026-08-27**, on PR [#60](https://github.com/CAPeddle/ai-memory/pull/60) merging to `main` as `86473ac` (`Story: ST-088` verified parsing on the squash). That merge carried the sign-off itself, the corpus-wide sweep of every artifact that had recorded the now-discharged condition, and one test fix (`b36ab11`, a real race in `server/tests/awcp-node-client-lock.test.ts`). **This transition is the second of the two conditions the B–D milestone was gated on** — `awcp-strategy-baseline-2026-08.md` decisions 1 and 3 gate it on the host decision being taken *and* ST-088 closing. The host condition was discharged at sign-off; this closes the other, so **B–D is now unblocked** and `/gsd-new-milestone` may generate it, subject to the three constraints ST-097's entry records (the co-tenancy tax moves to ST-082 rather than disappearing; threading `PolicyScope` through the read side is a precondition of the adapter contract; and B–D must not assume ST-100's scoring outcome). ADR-016 §1's Consequences bullet and its revision 1.5 row both state the two-condition rule and can now be read as satisfied. Branch `docs/st088-adr016-signoff` deleted; recoverable at tag `archive/st088-adr016-signoff`.
...
- **Open for the PO, neither blocking:** ~~`FEATURE_WORKFLOW` hardcoded `"true"` on base `mcp` leaves an unauthenticated dashboard shell on `0.0.0.0:3000` for every `docker compose up -d` (`T-03-01-02`)~~ — **now owned by ST-102, filed 2026-08-28.** It was never picked up from here: the audit found it had been routed to Phase 4, which closed without receiving it. The exposure itself is unchanged and still open; what changed is that it has an ID instead of a bullet. ~~and `.planning/STATE.md` progress metadata now describes a 2-phase project against ROADMAP's 4, with `current_phase` exceeding `total_phases`~~ — **repaired 2026-08-27** (`c508737`); the frontmatter now reads 4/4 against ROADMAP's Progress table, and `/gsd-complete-milestone` archives the file rather than recomputing it.
...
> **Transferred to AWCP (2026-09-25).** By PO direction, AWCP owns its own requirements, architecture and stories in [CAPeddle/awcp](https://github.com/CAPeddle/awcp); ai-memory's board keeps only work that builds ai-memory. Each entry names the awcp issue that now carries it.

### ST-103: State the standalone AWCP service requirements
- **Transferred to AWCP 2026-09-25:** the requirements now live at `docs/requirements/st-103-standalone-service-requirements.md` in [CAPeddle/awcp](https://github.com/CAPeddle/awcp) (imported by [PR #24](https://github.com/CAPeddle/awcp/pull/24)). This story never had an active board entry here; this record anchors its history.
- Plan: [docs/plans/2026-08-28-1212-chore-st103-awcp-standalone-service-requirements-plan.md](../../docs/plans/2026-08-28-1212-chore-st103-awcp-standalone-service-requirements-plan.md) (now a stub pointing at awcp; the last full text is commit `5cf5074`)

### ST-100: Score the standalone AWCP peer-service topology against the six host criteria
- **Transferred to AWCP 2026-09-25:** [CAPeddle/awcp#19](https://github.com/CAPeddle/awcp/issues/19), by PO direction that AWCP owns this work and ai-memory keeps only work that builds ai-memory. The entry below is retained as it stood at transfer, for `git log --grep` and traceability.
- Type: spike / architecture evaluation (produces the scoring ADR-016 §1(b) directs; the *select* half is closed by the PO on this evidence, not by this story)
- Source: carried forward from ST-088 explicitly rather than closed with it — `.github/planning/story-board.md`, ST-088's *"Carried forward, deliberately not closed here"* bullet, item (1), which also states that this work **needs its own story and a minted `ST-NNN`**. Named as the single largest gap in the *select* half of the recommendation at [docs/investigations/ST-084-awcp-host-spike-findings.md](../../docs/investigations/ST-084-awcp-host-spike-findings.md) §18.9 and §18.11, and **directed rather than concluded** by [ADR-016](../../docs/design/adr/ADR-016-awcp-consolidation-host-topology.md) §1(b) at the 2026-08-26 sign-off. The PO supplied a concrete topology to score at sign-off (`docs/investigations/Product stack.txt`)
- phase: 0 (architecture)
- Value: 4
- Blocked by: **nothing — unblocked 2026-08-27.** This story's entire premise was the ADR-016 sign-off landing on `main`, and it has: PR [#60](https://github.com/CAPeddle/ai-memory/pull/60) merged as `86473ac`, so `main` now carries ADR-016 → Accepted rev 1.5, ADR-013 → rev 1.3, findings §18.10 → APPLIED, and ST-088 → **Done** (it closed in the same pass, so the "Review, not Done" caveat below is discharged too). *(Historical: this line previously said "unblocked the moment that branch merges — until then `main` still reads `status: "proposed"`, so verify before grounding anything on it." The verification it asked for has been done; keep re-verifying `file:line` citations per the Notes, which is a separate caution and still live.)* `ST-100` was minted through `./story-id.sh` and its allocator line is on `main`, so the ID is usable. Deliberately **not** blocked on **ST-082** — criterion 5 is an *input* to the scoring, not a prerequisite for it. Not blocked on **ST-088** either, though note ST-088 sits in **Review, not Done**, precisely because it carried this gap forward rather than filling it
- Touches: `docs/investigations/awcp-ST-100-peer-service-topology-scoring.md` (new; no such file exists yet); no code changes expected. **Reads but does not modify** `docs/design/adr/ADR-016-awcp-consolidation-host-topology.md` §1, `docs/investigations/awcp-spec-evaluation.md` §7, `docs/investigations/ST-084-awcp-host-spike-findings.md` §18, `server/src/workflow/ports.ts`, `server/tests/workflow-boundary.test.ts`
- Acceptance criteria:
  - [ ] **Re-verify the ground before citing it — and note that this entry had to take its own advice.** Every anchor into a file the sign-off touched moved, and an earlier draft of this criterion asserted some of them were "stable across both trees". They were not: within the sign-off PR alone, findings §18.9 went `:1958` → `:1964`, §18.11 `:2048` → `:2071`, and `awcp-spec-evaluation.md`'s six-criteria and Candidate-C rows `:179`/`:185` → `:203`/`:209` — each review round shifted them again. **So this entry now cites `ST-084-awcp-host-spike-findings.md`, `awcp-spec-evaluation.md`, ADR-016 and the board by §-anchor only.** Line cites are kept solely for files the sign-off never edited and which were re-verified at filing: `server/src/workflow/ports.ts:109` / `:162`, `prism-ground-truth-inventory.md:144`, `awcp-external-evidence-import-2026-08.md:63` / `:84`, `awcp-strategy-baseline-2026-08.md:114-129` (the six decisions), and ADR-013 `:94` / `:102` / `:108` / `:110` / `:118` / `:120`. **Re-check even those** — the lesson generalises, and findings §12a is the standing worked example of cited claims moving underneath a later reader
  - [ ] **The subject is defined concretely before it is scored, and it is the PO's target stack — not an abstract "standalone service".** The topology of record is `docs/investigations/Product stack.txt`: three coordinating domains above AWCP — **Household, Dev projects, Trading R&D** (`:1-7`, corroborated by ADR-016 §1's tenancy paragraph) — and three capability providers below it (`:13-17`): **ai-memory** (knowledge), **Agent Radio** (agent routing, fanning out to Claude / Codex / OpenCode / Copilot / Local), and **Verification**. The provider triple is what findings §18.11 records the PO as supplying, *"which gives the scoring work a defined topology to score rather than an absence"*; the capability labels come from ADR-016 §1's interface paragraph. Reproduce the stack in the findings doc so the scored subject is unambiguous
  - [ ] **The output shape is stated before any row is written.** No numeric scoring exists anywhere in this record: ADR-016 §1's retained table is **one prose cell per (criterion, candidate)**. Produce a prose column in the same shape — **same criterion names, same order, one row per criterion** — placed beside the retained cells so it is directly comparable rather than a differently-shaped table set next to them. **Candidate C's column is quoted as written and not re-purposed**: C is defined by donor retirement, which this topology does not do, so C's cells describe a different option and must not stand in as the peer-service's proxy
  - [ ] **Scored against the same six criteria, reproduced exactly, that A, B and C each received** — **domain fit, security model, code maturity, migration effort, operational simplicity, retirement path** (`docs/investigations/awcp-spec-evaluation.md` §7; the frame ADR-016 §1's retained **A-vs-C** table uses, and the frame `prism-ground-truth-inventory.md` §4 used to score B). **Three candidates, not four** — Candidate D was rejected by PO direction (`awcp-spec-evaluation.md` §7) and never scored; findings §18.9 and §18.11 both say "A, B, and C". Any claim that D was scored is unsupported
  - [ ] **"Migration effort" is scored *structurally*, never in day-counts — and the resolution is stated in the doc, not assumed.** Standing PO direction is that **effort is not an input to architecture evaluation** (ADR-016 §1, *"always discount time in evaluation… good design and functional fit over effort"*). The criterion is retained for comparability and scored on *what must be stood up, what relocates, and which boundaries change* — the register the existing C cell already uses (*"new repo, deploy, auth, schema"*). Findings §18.9 records the cross-topology day-count as **withdrawn, not merely unfilled**, and *"a defended extraction estimate is follow-on work"*. An extraction estimate is a **planning artifact this story may name as a gap; it is not an input to the verdict**, and no day-count may appear in the scoring — including the *"64+ hours / 8+ days"* figure ADR-016 §1 still carries for criterion 5, which is ST-082's pricing and not a scoring input here
  - [ ] **The two PO additions of 2026-08-26 are carried in as scoring inputs, because A, B and C were never scored against them.** (i) **Interface replaceability** — capability providers are named by what AWCP needs from them, not by the product supplying them; the port interface is the **contract**, the wire protocol (in-process today, MCP or A2A later) is an adapter detail deliberately left open; ai-memory is *a* knowledge provider, not *the* knowledge layer. (ii) **The tenancy ground** — AWCP coordinates household, development-project and trading-R&D work, and under Candidate A all three would live inside a memory product's codebase, schema and Postgres role; ADR-016 §1 records this as a ground the spike did not produce. Both bear on domain fit, security model and retirement path; scoring that omits them just re-shuffles July evidence
  - [ ] **Code maturity is scored on observed state, and the extraction sketch is not treated as evidence.** The observed facts are findings §18.5's ownership inventory — every file under `server/src/workflow/` is AWCP-owned (`api`, `attention`, `dashboard`, `observedSession`, `policy`, `readModel`, `remoteNodeHub`, `schema`, `service`, `store`, `types`, `workItemStore`, `bootstrap`), plus `server/scripts/awcp.ts` and `server/scripts/awcp-node-client.mjs`, with `ports.ts` as the seam — and the import-boundary proof that no other file imports that seam (`server/tests/workflow-boundary.test.ts`, cited at §18.8 Phase A). **§18.8 Phase C's *"move as a unit … only relocating"* is a sketch, not a measurement** — §18.8 is headlined *"sketch only, **not started**"* and opens *"Nothing below is executed by this commit."* Cite it as an untested hypothesis or not at all. **Do not assert in advance that this row comes out distinct or favourable**; asserting a row's conclusion before writing it is the overclaim findings §18.4 and §18.9 already had to correct
  - [ ] **Security model is scored against the read-side scope gap, with its owner named.** `KnowledgeSearchPort.search(query, limit)` (`server/src/workflow/ports.ts:109`) carries **no** `PolicyScope` while `PromotionInput.policyScope` (`:162`) is typed to the closed union — both verified against the current tree. ADR-016 §1 makes threading scope through the read side, **default-deny**, a *precondition of the adapter contract*, and findings §18.6 warns that *"collapsing 15 enforcement points to one only helps if the one is actually enforced"*. The scoring must reflect that the single-chokepoint benefit is **prospective, not banked**, that the underlying obligation is **ST-082's** and **neutral between topologies**, and that criterion 5 is **not** discharged by ADR-016
  - [ ] **Agent Radio and Verification are scored as replaceable capability *slots*, not as products.** What is evaluated is AWCP's ability to consume an agent-routing capability and a verification capability behind contracts that survive the provider being swapped — not whether `cpeddle/agent-radio` or any particular verification tool is the right one. Where agent-radio's evidence is referenced at all, state it in the exact shape `docs/investigations/awcp-external-evidence-import-2026-08.md` uses (§ *"Scope of the STOP"*, `:63`, `:84`): **in-flight injection and steering are measured-negative and stopped; the synchronous-turn path is *unresolved* — its only PASS was withdrawn by its own authors and the corrective live run has not passed.** Neither "agent-radio is halted" nor "agent-radio works" survives that document, and neither reading may be used as a scoring input. Verification has no artifact at all; say so rather than scoring it by assumption
  - [ ] **All five §18.9 gaps are dispositioned explicitly — silence on any of them fails this criterion.** (a) **no load/concurrency evidence for AWCP standalone** — likely re-declared, since §18.9 itself scopes the decision to coupling rather than capacity, but the doc must say so and name it as Phase B infrastructure-sizing work; (b) **no extraction estimate** — re-declared as a **planning** gap, deliberately outside the verdict per the migration-effort criterion above; (c) **the topology is unscored** — this is the story's own subject and is closed by doing the work; (d) **no topology-specific cost figure, and none now sought** — §18.9 records it as no longer a gap in the recommendation under the standing effort direction, but still a planning gap; restate that disposition rather than reopening it; (e) **the withdrawn cross-topology day-count** — re-declared as withdrawn, with the doc stating that anything citing an A/C effort delta is citing a withdrawn figure. Plus the Horizon B / agent-radio relationship, which §18.4 and §18.9 already qualify as **asserted from `awcp-strategy-baseline-2026-08.md`, not independently re-verified** — carry that qualification forward unchanged
  - [ ] **An honest verdict from the bounded outcome set {CONFIRM the direction / QUALIFY it / CONTRADICT it}**, stated in those words, with the reasoning that produced it. Individual criteria may honestly be recorded **"insufficient evidence"** without that failing the story — that is §18.9's own idiom — but the overall verdict must be one of the three. **Contradiction is a legitimate, non-embarrassing outcome and the doc must be written so it can be reached** — the burden of proof sits with the spike, not with the preference. ST-088 is the precedent and must be cited as such: it **rejected its own preferred hypothesis** (Candidate A), and its outcome was one its own board Notes had not anticipated
  - [ ] **A CONTRADICT or QUALIFY verdict returns to the PO as a proposed ADR-016 revision — never as a unilateral reversal.** The doc drafts the proposed replacement text and stops there, the way findings §18.10 drafted ADR-016 §1's text and left it for sign-off. **Part (a) of ADR-016 §1 — the rejection of Candidate A — is settled on the full ST-084/ST-088 evidence and is NOT re-litigated by this story.** The subject is part (b) only
  - [ ] Out of scope, stated in the findings doc so a later reader does not assume otherwise: **no extraction work**, **no file moves**, **no edits to ADR-016** (a revision may be *proposed*, not applied), **no wire-protocol selection** (deliberately left open by ADR-016 §1), **no scoring of Agent Radio or Verification as products**, and no re-opening of criterion 5's implementation (ST-082) or repo-rescan (findings §19.2, owner *"unassigned at the U3/AWCP-roadmap level"*)
- Plan: (to be created — `docs/plans/`, with `story:` frontmatter carrying the minted ID and this entry's `Plan:` pointing back at it)
- Docs: [docs/investigations/ST-084-awcp-host-spike-findings.md](../../docs/investigations/ST-084-awcp-host-spike-findings.md) §18.3 (the ports seam and its read-side scope gap), §18.4 (the recommended topology, and why it is **not** Candidate C), §18.5 (domain ownership inventory), §18.6 (the one-chokepoint caveat), §18.8 (bounded extraction roadmap A–E, **sketch only, not started**), §18.9 (missing evidence), §18.11 (verdict and the PO-supplied target stack — **§-anchor only; this section moved in the sign-off change**), §13.5 (the withdrawn cross-topology day-count), §19.2 (repo-rescan, carried forward with an owner); [docs/design/adr/ADR-016-awcp-consolidation-host-topology.md](../../docs/design/adr/ADR-016-awcp-consolidation-host-topology.md) §1 (the two-part decision, the interface requirement, the tenancy ground, and the retained **A-vs-C** scoring table — **§-anchor only**); [docs/investigations/awcp-spec-evaluation.md](../../docs/investigations/awcp-spec-evaluation.md) §7 (the six criteria; C's donor-retirement condition; D rejected by PO direction, never scored) — cited by §-anchor because this PR shifted the file's line numbers; [docs/investigations/prism-ground-truth-inventory.md](../../docs/investigations/prism-ground-truth-inventory.md) §4 (Candidate B scored on the same six criteria) and §5 (`:144` — the wiki as *"Frozen, not donated"*, and `:8`'s fact/interpretation caveat on that disposition); [docs/investigations/awcp-external-evidence-import-2026-08.md](../../docs/investigations/awcp-external-evidence-import-2026-08.md) `:63`, `:84` (agent-radio evidence and the exact scope of its STOP); [docs/design/adr/ADR-013-platform-product-definitions.md](../../docs/design/adr/ADR-013-platform-product-definitions.md) `:94` (platform-vs-product litmus), `:96`/`:102` (product register; the Workflow/Operations row as updated at revision 1.3), `:110` ("Layering is not deployment"); [docs/investigations/awcp-strategy-baseline-2026-08.md](../../docs/investigations/awcp-strategy-baseline-2026-08.md) `:114-129` (the six decisions; 1 and 3 are the ones that bind); `docs/investigations/Product stack.txt` (the PO's target stack, committed alongside the sign-off because the scoring rests on it); `server/src/workflow/ports.ts:109`, `:162`; `server/tests/workflow-boundary.test.ts`
- Relates to: ST-088 (produced the gap and named it; **now Done, closed 2026-08-27** — it carried the gap forward deliberately rather than filling it, which is why closing it did not close this), ST-082 (owns criterion 5's enforcement — an input to the security-model row, not a blocker on this story), ST-096 / ST-097 (AWCP milestone and horizon work sitting downstream of the *select* half being settled)
- Notes: **This story exists because ADR-016 §1(b) is a direction, not a scored selection, and says so in its own decision text.** The reject half (Candidate A) carries the full weight of the ST-084/ST-088 spike — criteria 1–4 and 6 met, criterion 5 outstanding and **neutral between topologies**, criterion 7 answered **no**. The select half was argued from that same evidence but never put through the six-criteria scoring **A, B and C** each received. It is also **explicitly not Candidate C**: C is defined by retiring both donors (`awcp-spec-evaluation.md` §7, the candidate table's Candidate-C row), and ai-memory stays live as a supported optional provider — calling it C would put a signature on an option whose defining condition it does not meet. **The burden of proof sits with the spike, not the preference.** ST-088's own Notes line held that bar and the spike rejected its preferred hypothesis; the same bar applies here, and a CONTRADICT verdict against a direction the PO has already signed is the outcome this story must stay able to reach. Recording it as "confirmed" without doing the work would repeat precisely the overclaim findings §18.4 had to correct — inventing a score for a topology in the same pass that proposes it. **Two mechanical cautions for whoever picks this up:** (1) ~~**confirm the sign-off is on `main`.**~~ **Done 2026-08-27** — PR #60 merged as `86473ac`, so `main` carries ADR-016 Accepted/rev 1.5, ADR-013 rev 1.3, findings §18.10's APPLIED marker, and ST-088 in **Done** (it closed with that merge rather than stopping at Review). *(Originally: "It was committed on `docs/st088-adr016-signoff`, not merged at filing… If `main` still reads `status: \"proposed\"`, this story has not started.")* (2) **Re-verify every `file:line` before use.** ADR-016 §1's criterion 6 moved `:54` → `:85` and findings §18.11 moved `:2048` → `:2065` in the sign-off change alone; findings §12a is the standing worked example of cited claims moving underneath a later reader.

### ST-097: Transition — the WorkItem contract (D0), the GSD/CE workflow migration (A), and the first AWCP slice (B)
- **Transferred to AWCP 2026-09-25:** [CAPeddle/awcp#20](https://github.com/CAPeddle/awcp/issues/20), by PO direction that AWCP owns this work and ai-memory keeps only work that builds ai-memory. The entry below is retained as it stood at transfer, for `git log --grep` and traceability.
- **Split 2026-09-25:** D0 and Workstream B stay with AWCP (awcp#20). Workstream A's unfinished units, A4–A7, are ai-memory's own planning migration and returned to ai-memory under **ST-095**, which now owns them. A1–A3 had already landed here.
...
- Value: 5
- Blocked by: **nothing is blocked on a decision.** The ADR-016 §1 override for migration `005` was **granted 2026-08-24** (narrow — that migration only), and the §3 storage-layout question is settled (existing `workflow` schema; WorkItem is the parent of WorkPacket in the same aggregate). D0-4 now *records* the override and must land before B2. ~~**A4–A7 remain gated on ST-088 closing**~~ — **ungated 2026-08-27: ST-088 is Done** (PR #60, `86473ac`), so the milestone boundary at which the new structure becomes authoritative has arrived and A4–A7 are runnable. Everything else, including the whole AWCP product slice, is runnable now
...
- Handoff:
  - **Read in this order:** (1) this story's plan, especially its **D0 verdict** and **Review disposition** table; (2) the strategy baseline's six decisions and its *"What is blocked, and on what"*; (3) `awcp-spec-evaluation.md:159-167` (the authority matrix) and `:177` (storage layout open, with its named process); (4) `ADR-016:88`
...
- **Transferred to AWCP 2026-09-25:** [CAPeddle/awcp#21](https://github.com/CAPeddle/awcp/issues/21), by PO direction that AWCP owns this work and ai-memory keeps only work that builds ai-memory. The entry below is retained as it stood at transfer, for `git log --grep` and traceability.
...
- Value: 3
- Blocked by: **ST-088** — baseline decision 3 puts the first new milestone at Horizon B, *after* ST-088 closes and ADR-016 leaves Proposed/Conditional. **Half-discharged 2026-08-26**, then **fully discharged 2026-08-27:** ADR-016 left Proposed/Conditional — it is **Accepted** (rev 1.5) with Candidate A rejected — and ST-088 has now closed (Review → Done, PR #60 merged as `86473ac`). **Both conditions are met and nothing blocks this story.** *(Historical: this line previously read "only 'ST-088 closes' remains, and ST-088 is in Review. Nothing here may be executed before that.")* The plan exists so the sequence is written down rather than rediscovered
...
- Acceptance criteria:
  ...
  - [ ] **The ADR-016 verdict selects between two conditional milestone shapes** (see the plan's Open Questions). Accept A pulls the co-tenancy tax forward; Recommend C pulls extraction/donor work forward. Neither is pre-chosen. **Answered 2026-08-26 — and it was neither shape.** ADR-016 rev 1.5 rejects Candidate A and directs a **standalone peer service**: extraction/donor work is pulled forward **without** donor retirement, so this is not the Candidate C shape either. Three constraints on the milestone that follows: the co-tenancy tax (`scope.tags` enforcement) does **not** disappear — it moves to ST-082 as ai-memory's own obligation, topology-neutral; threading `PolicyScope` through the read side is a **precondition** of the adapter contract, not follow-on work; and the peer-service topology is **unscored**, so B–D must not assume its scoring outcome
...
- Handoff:
  ...
  - **The presumed-host trap, in one line:** *"one deployable system containing different domains/modules"* **is** Candidate A, and `ADR-016:88` says *"no schema or migration work may assume the host"* until the spike concludes. **Updated 2026-08-26 — the trap inverted rather than lifted.** The spike concluded and the bar is discharged (`ADR-016:90`), but it concluded by **rejecting Candidate A**: assuming one deployable system is now not merely premature, it is contrary to an Accepted ADR. Schema and migration work no longer returns for a per-item host decision; it must instead assume AWCP is a **standalone peer service**, and it must not assume the peer-service topology's *scoring* outcome, which is still outstanding. The sharper form is the ADR's own: Candidate A's reuse is *"still zero"* for session instrumentation and approval semantics — which is exactly what horizons B and F propose to build
...
- **Transferred to AWCP 2026-09-25:** [CAPeddle/awcp#22](https://github.com/CAPeddle/awcp/issues/22), by PO direction that AWCP owns this work and ai-memory keeps only work that builds ai-memory. The entry below is retained as it stood at transfer, for `git log --grep` and traceability. The code this story fixes still runs in ai-memory (`server/`, the extraction donor), so until extraction any fix lands here as an ai-memory change, tracked from the awcp issue.
- Type: security
...
### ST-102: Own the unauthenticated `/workflow` dashboard shell that `FEATURE_WORKFLOW="true"` on base `mcp` left exposed
- **Transferred to AWCP 2026-09-25:** [CAPeddle/awcp#23](https://github.com/CAPeddle/awcp/issues/23), by PO direction that AWCP owns this work and ai-memory keeps only work that builds ai-memory. The entry below is retained as it stood at transfer, for `git log --grep` and traceability. The code this story fixes still runs in ai-memory (`server/`, the extraction donor), so until extraction any fix lands here as an ai-memory change, tracked from the awcp issue.
- Type: security decision / dev-surface disposition (the deliverable is a **recorded decision**; a code change is one of its possible outcomes, not its premise)
- Source: **a designed escalation that was never received.** Phase 3's plan created the exposure knowingly and refused to let it close silently — threat-register row `T-03-01-02` ([`.planning/phases/03-node-client-reliable-delivery-regression-safety/03-01-PLAN.md:209`](../../.planning/phases/03-node-client-reliable-delivery-regression-safety/03-01-PLAN.md)) reads *"accept (this phase) + escalate … **Escalated, not closed** … Do not resolve this by silently leaving the flag on."* `03-01-SUMMARY.md:111` carries it under the heading *"Open Question Recorded, Not Resolved"*, and `03-VERIFICATION.md` lists it as human-verification item 3. It was then routed forward twice — this board at `:969` (*"`FEATURE_WORKFLOW` permanence (Phase 4 owns it)"*) and at `:914` (*"Open for the PO, neither blocking"*) — and **Phase 4 never received it**: the ROADMAP Phase 4 entry carries two explicit *"Carried forward out of this phase"* items and this is not among them. Surfaced by `/gsd-audit-milestone` as **W3** ([`.planning/v1.0-MILESTONE-AUDIT.md`](../../.planning/v1.0-MILESTONE-AUDIT.md)) on 2026-08-28, immediately before the ST-088 milestone would have archived it unowned a second time
- phase: 0 (decision), then optionally 1 (implementation)
- Value: 3
- Blocked by: **nothing.** ST-088 is Done; this is not a milestone gate and never was
- Touches: `docker-compose.yml` (`:55-58`, `:67`), `server/index.ts` (`:1262`), `.github/planning/story-board.md`. **Reads but does not necessarily modify** `docs/design/adr/ADR-016-awcp-consolidation-host-topology.md`, `.planning/phases/03-node-client-reliable-delivery-regression-safety/03-01-PLAN.md`
- Acceptance criteria:
  - [ ] **The observed facts are restated and re-verified before anything is decided** — all four were verified against the tree on 2026-08-28 and each must be re-checked, because findings §12a is the standing worked example of cited claims moving under a later reader. (i) `docker-compose.yml:58` sets `FEATURE_WORKFLOW: "true"` **hardcoded**, not `${FEATURE_WORKFLOW:-}`, on the base `mcp` service — before Phase 3's 03-01 the base compose contained the variable **zero** times. (ii) `docker-compose.yml:67` publishes `"3000:3000"` on **all** interfaces, whereas `mcp-test` at `:138` is `"127.0.0.1:3001:3000"`, loopback-only — the asymmetry is the exposure, not the flag alone. (iii) `server/index.ts:1262` is `app.get("/workflow", (c) => c.html(DASHBOARD_HTML))` with **no middleware**, unlike `/api/workflow/*` at `:1212` and `/workflow/nodes/*` at `:1252`, which each have one. (iv) Consequently `GET http://0.0.0.0:3000/workflow` answers **200 unauthenticated** for every plain `docker compose up -d`
  - [ ] **The blast radius is stated accurately, in both directions, and neither half is dropped.** *No data leaks*: the shell is a static client shell and every byte it renders arrives through the authenticated `/api/workflow`, which correctly answers 401 — an anonymous visitor gets chrome, not data. *And it is still a real exposure*: it advertises the existence, shape and route surface of the workflow module to anything that can reach port 3000, on a bind that is not loopback. A decision that recites only the reassuring half fails this criterion
  - [ ] **The decision is one of a bounded set, chosen explicitly and written down**: (a) **accept** the exposure with stated reasoning and a stated scope (dev-only? and what enforces "dev-only" when the bind is `0.0.0.0`?); (b) **bind the shell** behind the same middleware `/api/workflow/*` already uses; (c) **narrow the publish** to `127.0.0.1:3000:3000`, matching `mcp-test`; (d) **un-hardcode the flag** back to `${FEATURE_WORKFLOW:-}` and require opt-in. Options (b), (c) and (d) are not mutually exclusive. **Do not resolve this by silently leaving the flag on** — that is the exact resolution `T-03-01-02` forbids by name, and the reason this story exists
  - [ ] **If (d) is chosen, the D-01 dependency is checked first.** Phase 3's D-01 required the flag on the base `mcp` service because the real-node leg pointed at the dev hub; reverting the flag must not silently re-break `POST /workflow/nodes/register` back to the **404** that `ce-doc-review` caught on 2026-08-15. Verify the flag's state **inside the running container** (`docker compose exec -T mcp printenv FEATURE_WORKFLOW`), never from `.env` or an HTTP response — `docker compose up -d mcp` can silently keep stale environment (observed 2026-08-18)
  - [ ] **`T-03-01-02` is closed by name**, in this entry and wherever else it is open, so the threat-register row stops reading as escalated-and-unanswered
  - [ ] **The two routing sites that sent this to Phase 4 are corrected**, not left pointing at a phase that closed without it: this board `:969` and `:914`. Marked retention — strike or quote the superseded text in place rather than deleting it, so an old citation still resolves
- Plan: (to be created if the chosen option needs one — option (a) needs only a recorded decision)
- Docs: [`.planning/v1.0-MILESTONE-AUDIT.md`](../../.planning/v1.0-MILESTONE-AUDIT.md) (W3, and the audit that found it); [`.planning/phases/03-node-client-reliable-delivery-regression-safety/03-01-PLAN.md`](../../.planning/phases/03-node-client-reliable-delivery-regression-safety/03-01-PLAN.md) `:209` (`T-03-01-02`), `:163`, `:193` (the open-questions block that made this a maintainer decision rather than an executor's call); `03-01-SUMMARY.md:111-117`; `03-VERIFICATION.md` (human item 3); `03-REVIEW.md:533`; [ADR-016](../../docs/design/adr/ADR-016-awcp-consolidation-host-topology.md) `:184` — which dispositions the flag's **existence** (*"stays until an extraction plan retires them"*) and **not** this exposure; those are different questions and the ADR must not be cited as having settled this one
- Relates to: ST-088 (created the exposure in Phase 3 and was the phase that should have received the escalation; now **Done**, which is why this needed its own ID rather than a reopened story), ST-100 (the peer-service extraction may retire `FEATURE_WORKFLOW` entirely — but *may*, on an unscored topology, is not a reason to leave a live exposure unowned in the meantime)
- Notes: **Filed 2026-08-28 by `/gsd-audit-milestone`, before `/gsd-complete-milestone` ran.** This is the whole return on running the audit: nothing else in the milestone was undelivered, no cross-phase link was broken, and the only genuinely dropped item was a handoff whose own paperwork had said in advance that it must not be dropped. **The class, worth naming because it will recur:** an item routed forward with a named owner ("Phase 4 owns it") is only as good as the receiving artifact's willingness to list it — Phase 4's *"Carried forward"* block was written from Phase 4's own findings, not from a sweep of what upstream had addressed to it. A routed item needs a *destination that enumerates its inbox*, or an ID of its own. This one now has an ID. **A sibling from the same review is still unowned and deliberately out of scope here**: `last_seen_at` never advancing on ingestion (this board `:969`, *"hub-side, worth its own story"*) — it makes a healthily-reporting node read as stale. Same class, different defect; it wants its own minted ID rather than being absorbed into this one
