...
- Acceptance criteria:
  - [ ] `graph_traverse` comment-handling: 3 `assertEquals` failures at `cypher-injection.test.ts` L114/127/140 (keyword inside line comment, inside block comment, leading comment before `MATCH`) — determine whether the MATCH-only/comment-stripping guard changed behavior or the expectations are stale, and reconcile
  - [ ] `consolidation-worker-observability.test.ts:68` — `PostgresError: null value in column "query" of relation "recall_events" violates not-null constraint`: fix the missing `query` value (test fixture or code path)
  - [ ] worker-observability (2 failures) — `PostgresError: duplicate key value violates unique constraint "entity_extraction_queue_pkey"`: almost certainly a test-isolation/fixture-collision issue against the shared `db-test` corpus; ensure unique PKs or proper cleanup between tests
  - [ ] Full `deno test tests/` goes green (currently 219 passed / 6 failed)
- Plan: (to be created — `docs/plans/`)
- Notes: Surfaced 2026-07-03 the first time the integration suite ever ran in CI. All 6 failures are in files **unchanged from `main`** (`git diff --name-only origin/main...HEAD -- server/` shows only the two ST-070 health files) — so they are pre-existing on `main`, not introduced by PR #21 or ST-070. Root story: the CI `integration-tests` job has been red since inception (secret gate → type error → these), so the suite's green state was never established. The duplicate-key and null-`query` errors smell like test-isolation problems against the seeded `db-test` container rather than product bugs; the `graph_traverse` ones may be genuine assertion/behavior drift. Needs triage per-failure. Blocks PR #21's checks going fully green.
