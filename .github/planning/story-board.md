...
- **Carried forward, deliberately not closed here:** (1) the **peer-service topology is unscored** against the six criteria A/B/C each received — findings §18.9 calls it the largest gap in the *select* half, and ADR-016 §1(b) directs the scoring rather than concluding it; needs its own story and a minted `ST-NNN`. (2) **Repo-rescan** stays a real U3 capability gap with its design direction recorded (findings §19.2). (3) **Criterion 5** moves to **ST-082**, reframed as ai-memory's own obligation rather than a co-tenancy tax. (4) **prism-llm-wiki knowledge migration** into ai-memory — a PO decision of 2026-08-26 that reverses the wiki's "frozen, not donated" disposition for content only, driven from the `prism-llm-wiki` side through ai-memory's capture tooling; separate story, unrelated to ST-088's scope.
