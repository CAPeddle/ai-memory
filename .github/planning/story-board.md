...
- Acceptance criteria:
  ...
  - [ ] **Criterion 7 — final extraction viability and the final ADR-016 recommendation.** Extraction-viability judgment complete and a full recommendation drafted 2026-08-26 — findings `docs/investigations/ST-084-awcp-host-spike-findings.md` §18: criteria 1-6 met, criterion 7 (does ai-memory's reuse justify Candidate A's domain-fit cost) answered **no**, recommendation is reject Candidate A / select Candidate C (standalone AWCP, ai-memory as optional context provider via the existing `ports.ts` adapter contract). Box stays unchecked because the recommendation is **held for PO sign-off, not yet applied to ADR-016** — its live `status` and Decision text are untouched, per explicit PO direction this session. §18.10 carries the exact proposed replacement text
