...
export function isContextError(
  result: ContextParseResult | null,
): result is ContextParseError {
  return result !== null && "error" in result && result.error === true;
}

export function parseContext(
  raw: string | undefined,
): ContextParseResult | null {
  if (!raw) return null;
  ...
  for (const pair of raw.split(",")) {
    ...
    if (colonIdx === -1) {
      return {
        ...
        received: raw,
        expected:
          'Comma-separated key:value pairs. Example: "project:myapp,tags:developer;contact,strict"',
        failedToken: trimmed,
      };
    }
    ...
    if (!VALID_KEYS.has(k)) {
      return {
        error: true,
        message: `Unknown key "${k}" — valid keys: ${
          [...VALID_KEYS].join(", ")
        }`,
        received: raw,
        expected:
          'Comma-separated key:value pairs. Example: "project:myapp,tags:developer;contact"',
        failedToken: trimmed,
      };
    }
    ...
    if (k === "project") scope.projects = v.split(";");
      ...
      const parsedTags = parseTagList(v);
      if (isTagValidationError(parsedTags)) {
        return {
          ...parsedTags,
          received: raw,
          failedToken: trimmed,
      ...
      if (!VALID_VISIBILITIES.has(v)) {
        return {
          error: true,
          message:
            `Invalid visibility "${v}" — must be "prefer", "exclusive", or "cross-only"`,
          received: raw,
          expected:
            '"visibility:prefer", "visibility:exclusive", or "visibility:cross-only"',
          failedToken: trimmed,
        };
      }
...
export function parseContextOrError(
  raw: string | undefined,
): ContextScope | null | {
  ...
  if (isContextError(result)) {
    return {
      content: [{
        type: "text" as const,
        text:
          `Context validation error: ${result.message}\nExpected: ${result.expected}\nReceived: "${result.received}"`,
      }],
      isError: true,
    };
  }
