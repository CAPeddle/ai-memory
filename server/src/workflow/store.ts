 ...
 *
 * This module and its sibling `workItemStore.ts` (ST-098 U2 — WorkItem persistence,
 * split out of this file) are the only workflow files that import the database
 * handle, alongside `schema.ts`. Every statement here is schema-qualified
 * `workflow.*` — mandatory, because AGE graph queries leave a sticky polluted
 * `search_path` on pooled connections (the
