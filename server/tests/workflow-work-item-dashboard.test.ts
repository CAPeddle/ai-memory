...
// ---------------------------------------------------------------------------
// The shim: the smallest DOM the dashboard's script touches
// ---------------------------------------------------------------------------
...
function markup(node: ShimNode): string {
...
function byClass(node: ShimNode, cls: string): ShimNode[] {
 ...
 * and shadowing `fetch` by parameter is what keeps this test off the network.
 */
function bootDashboard(overview: unknown): { root: ShimNode; load: () => Promise<void> } {
