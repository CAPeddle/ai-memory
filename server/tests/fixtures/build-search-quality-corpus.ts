...
const sqlLines: string[] = [
  ...
  "-- Disable the consolidation-queue trigger during seeding so the background worker",
  "-- cannot deactivate corpus shards before tests run (SET active = false).",
  "ALTER TABLE public.thoughts DISABLE TRIGGER trg_queue_consolidation;",
...
sqlLines.push(
  "-- Re-enable the trigger now that seeding is complete.",
  "ALTER TABLE public.thoughts ENABLE TRIGGER trg_queue_consolidation;",
);
