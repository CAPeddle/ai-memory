---
name: project-deno-in-container
description: "Deno runs inside the `mcp` Docker container, not on the host. ExecPlan commands should use `docker compose exec mcp deno ...`"
metadata: 
  node_type: memory
  type: project
...
Same pattern applies to `psql`: use `docker compose exec db psql ...` rather than assuming a host psql install. See [[project-execplan-verification-scope]] for the general principle.
