...
- **Write order: repo-local OpenCode files first, then Windows env, then verify-all.** Surface B (repo-local, narrow blast radius) before Surface A (user-wide, broad blast radius). OpenCode rewrites use a co-located temp-file → SHA-256 verify temp → `mv` (atomic same-filesystem rename, per file). The for-loop interleaves verify and echo between renames, so the cross-file kill-window is wider than a bare "two syscalls" — in practice ~6 syscalls plus subshell spawns. Convergence is unaffected because classification is per-file (the file written first is `already-target` on the re-run; the unwritten one is still `placeholder`/`missing` → rewritten). If Surface B fails, Surface A never runs (no spurious VS Code restart signal). The two surfaces are logically independent: VS Code reads the Windows env, opencode reads the real files — a partial write *across* A and B does not break either consumer.
...
│   │   └── temp-file -> SHA-256 verify temp -> mv (atomic same-FS rename per file; cross-file window ~6 syscalls, convergence preserved per-file)
...
**Convergence invariant:** Re-running the script after an interrupted prior run leaves every managed surface in the state dictated by the current `.env`, regardless of which subset of writes the prior run completed — provided Phase 0 preflight is clean. No surface requires manual intervention unless a *pre-existing divergent* key was present before the first run, and the script never introduces divergence itself. Global all-or-nothing atomicity is **not** claimed (no shared transaction coordinator across the Windows registry and WSL files); each per-file write is an atomic same-filesystem `mv` (temp-file co-located via `mktemp -p`), so a SIGKILL between files leaves each file either unchanged or fully written — never truncated and un-healable. Residual risk is a cross-file kill-window during Phase 1 (widened by intervening verify/echo steps, not a bare "two syscalls") and a latency-width window between Surface B and the powershell call, both self-healing on the next run.
...
- **Phase 1 (Surface B — repo-local OpenCode files):** for each real file, copy its `.example` template → temp file (co-located via `mktemp -p` for an atomic same-filesystem `mv`), inject the target key, SHA-256 verify the temp, then `mv` into place. The per-file `mv` is atomic; the cross-file kill-window (between file 1's write and file 2's write) is widened by the intervening grep/echo/verify steps, but convergence is preserved per-file on re-run. For `already-target` files: SKIP the write entirely (no EOL churn). For a missing real file: create from template + inject. If any Phase 1 step fails, abort before Phase 2 (Windows env untouched → no spurious restart signal).
...
- **State lifecycle risks:** Global all-or-nothing atomicity is **not** claimed (no shared transaction coordinator across the Windows registry and WSL files). **Convergence invariant:** after a clean Phase 0 preflight, re-running the script converges all managed surfaces (real OpenCode files, Windows env) to the `.env`-dictated state regardless of which subset of writes a prior interrupted run completed — each per-file write uses an atomic same-filesystem rename (temp-file co-located via `mktemp -p "$(dirname "$f")"`), so a SIGKILL between files leaves each file either unchanged or fully written, never truncated; re-run classifies it correctly and converges. The only un-healable state is a *pre-existing divergent* real key, which fails loud by design (Phase 0, before any write). Residual: a cross-file kill-window during Phase 1 (widened by intervening verify/echo steps) and a latency-width window between Surface B and the powershell call — both self-healing on the next run. Write order Surface B (repo-local) before Surface A (broad blast radius) means a failed Surface B never triggers a spurious Windows env change.
