...
- KD4. Capability providers are replaceable slots, named by what AWCP needs from them rather than by the product supplying them. The port interface is the contract; the wire protocol stays open. Governs R9, R10, R11, R63, R65.
...
- R63. [INV] The agent-routing slot's verb set is the ceiling of what any adapter may be asked for, and each verb carries its evidence status: session observation is confirmed; turn dispatch is unresolved, its only measured pass withdrawn by its own authors; in-flight steering and interruption are measured-negative and excluded from the slot until re-measured. Naming a verb is not selecting its wire form (R11), and R15's per-verb obligations quantify over this set rather than over an unnamed one. What a given adapter actually answers to is narrower still, and R65 settles that. *(session-settled: user-directed — external review asked for a seven-verb taxonomy; two of those verbs are measured-negative and one is unresolved, so the set is stated with its evidence rather than adopted whole.)*
- R65. [INV] Each adapter declares which of the slot's verbs it supports, and the service issues only declared verbs. An undeclared verb resolves as unsupported — a third outcome under R15, distinct from accepted and from delivered — and a capability the adapter does not declare is never emulated in a way a caller cannot distinguish from the provider having performed it. Support is a property of the adapter in front of the service, not of the slot in the abstract, so two providers occupying one slot may answer to different subsets without either being non-conforming. *(session-settled: user-directed — external review, second round: without declaration the verb set reads as global, and adapters for different coding agents support different subsets.)*
...
- R15. [INV] A provider contract distinguishes accepted from delivered per verb, and authoritative from observed per state read, with an observability obligation on each. Unsupported is a third outcome, distinct from both, and R65 governs when it arises. A capability an adapter cannot verify it performed is not a capability.
...
- R34. [INV] Every external issue-tracker write passes a draft, preview, approve, execute ledger. The approval is permanently manual, non-delegable and non-replayable, and no agent may carry an instruction not to re-ask. An approval binds to the exact previewed payload: any change between preview and execution voids it and returns the item to draft. Each approval carries an execution identity bound to the approved payload and passed to the provider as the idempotency key, and execution resolves to R14's four outcomes: an unknown or reference-lost write is reconciled against the provider by that identity, never re-approved and re-executed, because R16 means a timed-out write may still have committed. Reconciliation discovers whether the one authorised write happened; it does not replay the approval, and only a failed outcome returns the item for a fresh approval. *(session-settled: user-directed — external review, third round: the approval ledger had no reconciliation path, so recovery could duplicate an external write; it takes the same treatment R14 gives Promotion.)*
...
- R36. [INV] The Spool is bounded, and the bound is held without losing causal history. Eviction consumes coalescible entries only — those a later unacknowledged entry with the same coalescing key supersedes, so the newest entry per key is never the one dropped — and each drop is counted and announced rather than lost. An unacknowledged non-coalescible entry is never evicted: where coalescing cannot free the room, the producer is observably blocked or refused, or the entry is durably spilled outside the Spool. Which of those a lane uses is open; silently dropping a non-coalescible entry is not among the options. Whether a type is coalescible, and its coalescing key — the type plus the node, session or subject identity whose latest state the entry carries — are declared with the type, so the classification cannot drift from the events it governs. Supersession is per key, never per type: two sessions on one node emit heartbeats of the same type, and one session's heartbeat must not evict the other's latest liveness. *(session-settled: user-directed — external review, second round: a bound can be held by backpressure or spill as well as by loss, so preferential eviction alone still permitted irreversible causal loss under sustained backpressure. The first round's coalescible-first ordering came from this document's own AE16 and AE17, which make heartbeats coalescible by design while F5 puts them on a fixed cadence. Third round: coalescing by type alone let one session's heartbeat evict another's, so supersession is scoped to a declared coalescing key.)*
...
- R42. [INV] An Evidence Item records the base commit it was verified against, a content identifier distinguishing pre-commit state from that commit when the check ran before one existed, its Verified Surface as paths, its verifier's identity, an immutable digest of the result, and the verifier method, tool and configuration version the adopted contract requires — so freshness is computed rather than asserted, evidence produced before a commit exists remains replayable and fresh-checkable, and replaying historical evidence is distinguishable from rerunning a nominally similar check. Verifier identity and source state alone do not make evidence replayable when the tool, its configuration or the stored result can change. Evidence that cannot name its surface in paths is evidence whose expiry cannot be detected. *(session-settled: user-directed — external review, third round: replayability needs the result digest and method version, not verifier identity alone.)*
- R64. [INV] Evidence the service itself retrieved from the verification slot is accepted against an already-adopted Criterion without operator disposition, because the service authenticated the source rather than trusting a report of it. Authentication establishes provenance and nothing more: acceptance additionally requires the retrieved result to be bound to the exact adopted verification request — its Criterion, its Verified Surface, the method and result semantics that contract states, and its freshness under R42 and R43 — and to satisfy R44's validity criteria. A correctly authenticated verifier answering a different question, covering a different surface, or reporting against a stale revision is refused on those grounds despite its provenance. Adopting the verification contract remains operator-only under R32. This is a distinction of provenance, not of trust level: the same result relayed through an Agent Run's report stays a claim under R45, so the exception cannot be reached by an agent presenting itself as a verifier. *(session-settled: user-directed — external review showed operator-only acceptance turns deterministic verification into a manual queue; the retrieval path was chosen over an inbound verifier credential, which would have reopened R23's closed class set.)*
- R43. [INV] Expiry means unobserved, not false, and has three modes that are reported distinctly: the surface moved, so the result no longer describes it; the surface grew, so the result still holds but under-covers; and the method moved, so the verifier method, tool or configuration version the contract requires is no longer the one the result was produced under, and the result can be replayed but not rerun as the same check.
...
- R47. [INV] Once-and-final adjudication is uniform across Operational Decision resolution and Agent Run closure: a repeat of the same answer returns the stored record with its original timestamp intact, and a different answer is refused as a conflict naming the stored one. Correction travels through supersession under R66, never through mutating the stored record.
- R66. [INV] An accepted Operational Decision resolution or Agent Run closure is superseded rather than corrected in place: an operator, and only an operator, records a new adjudication naming the superseded record and the reason, at most one successor is active for a given record at a time, and every prior record is retained. Consumers resolve the active adjudication; audit resolves the whole chain. This leaves R47 intact, because a superseding adjudication is a new record rather than a different answer to the old one. *(session-settled: user-directed — external review: finality should mean an immutable record, not an uncorrectable operational conclusion, or an erroneous closure cannot enter bounded rework except by violating R47.)*
...
- F4. Event delivery from a node
  ...
  - **Actors:** A3
  - **Steps:** append to the Spool; hold the bound by evicting a coalescible entry and counting the drop, or by refusing the producer or spilling durably when no coalescible entry is superseded under its key; flush a bounded batch oldest-first; the service refuses malformed or oversized payloads naming the offenders, verifies node ownership, ingests idempotently, and acknowledges by reading back; the node removes only entries named and sent; the response maps to exactly one dispatch.
  - **Outcome:** every event is delivered, still queued, spilled, refused at the producer, or counted as dropped — never merely absent, and a non-coalescible event is never among the dropped.
...
- AE25. **Covers R66, R47.** Given a resolved Operational Decision that an operator supersedes with a new adjudication naming it and a reason; Then the original record is retained unchanged, the successor is what consumers resolve, and a second successor is refused while the first is active. Re-resolving the original with a different answer is still refused under AE4 — supersession adds a record, it does not reopen one.
...
- AE26. **Covers R34, R16.** Given an approved issue-tracker write whose bounded wait elapsed with the outcome unknown; When recovery runs; Then it reconciles against the provider by the approval's execution identity and reports the write as committed or not, never requesting a new approval for the same payload nor issuing a second write under a new identity. Given the write committed but its reference was not recorded; Then the outcome is reference-lost, as in AE5. Given the provider explicitly declares nothing was committed; Then the outcome is failed and the item returns for a fresh approval, because the approval authorised one execution and is not replayable.
...
- AE11. **Covers R36.** Given a Spool at its bound holding both coalescible and non-coalescible entries; When a new event is produced; Then the event is appended, a coalescible entry superseded by a later entry with the same coalescing key is evicted, the drop counter increases, and the drop is announced. No non-coalescible entry is evicted while the bound can be held any other way, and two sessions' heartbeats on one node hold distinct keys, so neither supersedes the other.
- AE23. **Covers R36.** Given a Spool at its bound holding only unacknowledged non-coalescible entries, or coalescible entries whose coalescing keys are all distinct so that none is superseded; When a new event is produced; Then nothing is evicted — the producer is observably blocked or refused, or the entry is durably spilled outside the Spool. The bound still holds; it is held by backpressure or spill rather than by loss, and no non-coalescible entry is ever silently dropped.
- AE24. **Covers R64, R45.** Given a Criterion the operator already adopted and a result the service retrieved itself from the verification slot, bound to that Criterion's adopted request and fresh against its Verified Surface; Then it is accepted without operator disposition. Given a correctly authenticated result that names a different Criterion, covers a different surface, or reports against a stale revision; Then it is refused, because provenance is a precondition of acceptance and not a substitute for the contract being satisfied. Given the identical bound result presented instead through an Agent Run's report; Then it remains a claim.
...
- Q6. **Settled.** AWCP has its own repository, [CAPeddle/awcp](https://github.com/CAPeddle/awcp), as the canonical home for its code, requirements and ADRs, and this document transferred there on 2026-09-25 by operator direction, ahead of the extraction-authorisation gate. ai-memory retains the donor history and may supply an optional knowledge adapter, but does not remain a parallel authority for AWCP governance. *(session-settled: user-directed — the operator named the repository and then directed that AWCP take over this story, leaving ai-memory to build ai-memory. The transfer moves governance only: no code extraction or ST-100 verdict follows from it.)*
...
- Q8. **Deferred.** Which spill or backpressure mechanism does each remote lane use to hold the Spool's bound under R36? The requirement states the behaviour and deliberately leaves the mechanism open; an outbox, a dead-letter ledger and producer refusal all satisfy it, and they differ in operational cost rather than in contract.
