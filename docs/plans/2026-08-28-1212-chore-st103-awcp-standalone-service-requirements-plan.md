...
- F4. Event delivery from a node
  ...
  - **Actors:** A3
  - **Steps:** append to the Spool; hold the bound by evicting a coalescible entry and counting the drop, or by refusing the producer or spilling durably when no coalescible entry remains; flush a bounded batch oldest-first; the service refuses malformed or oversized payloads naming the offenders, verifies node ownership, ingests idempotently, and acknowledges by reading back; the node removes only entries named and sent; the response maps to exactly one dispatch.
