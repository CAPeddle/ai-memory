...
- Q6. **Settled.** AWCP has its own repository, [CAPeddle/awcp](https://github.com/CAPeddle/awcp), as the canonical home for its code, requirements and ADRs. This document moves there once the extraction-authorisation gate clears and stays filed here until then; ai-memory retains the donor history and may supply an optional knowledge adapter, but does not remain a parallel authority for AWCP governance. *(session-settled: user-directed — operator decision recorded on the review surface, confirmed by the operator naming the repository. It records ownership only: no file move, extraction or ST-100 verdict is authorised by it.)*
