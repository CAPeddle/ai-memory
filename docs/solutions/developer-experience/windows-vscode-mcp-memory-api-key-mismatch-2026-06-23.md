...
- `.vscode/mcp.json` configures VS Code MCP auth with `${env:MEMORY_API_KEY}`.
- `server/src/auth.ts` validates the exact Bearer header against server-side `MEMORY_API_KEY`.
- `README.md` documents the VS Code MCP path and should explicitly warn that the Windows env var must match repo `.env`.
- `docs/wsl2-setup.md` should include the Windows user environment synchronization step in the WSL2 setup checklist.
