...
- **Prism's real role is donor of three shipped mechanisms** the AWCP ladder needs at increments 4–5: external source lineage with content hashing (increment 5, built), hash-baseline drift detection with a solved false-stale problem (increment 4, mechanism built), and Jira↔git evidence correlation (increment 4/6, mechanism built). Different *semantics* in each case — see §5. Its **identity** is cross-boundary record correlation, not knowledge synthesis, which places those mechanisms inside AWCP's evidence layer rather than in a separate source/wiki product.
...
| The wiki itself — 60 content pages, index, log, conflict/archive rules | No AWCP counterpart | **Frozen, not donated** | Maintained synthesis competes with Rovo's query-time synthesis over the same sources and loses on index, freshness, and staleness burden. The 60 pages freeze at current content; the review is event-triggered on a page being wanted and unavailable |
