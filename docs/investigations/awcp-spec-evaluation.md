...
- **Overlapping subsystems that should be served by one shared engine, not rebuilt:**
  ...
  - **ExternalSnapshot / Confluence capture (FR-INT-003)** — confirmed overlap: Prism's Confluence/ADO ingestion is **built and in production** (350-page registry, SHA-256 content hashing, staleness tracking) — see the Prism ground-truth inventory (§5, §8 Q1 update).
...
- **FR-INT-003 (capture and version Confluence page content as source snapshots)** is not merely "presumably" Prism's home turf — it is **built and in production**: a 350-page Confluence registry with SHA-256 content hashing, version/last-scraped tracking, staleness queries, and page→wiki-page projection mapping, gated by a run-state machine. The requirement is still **one authoritative source lineage with no competing authoritative copies** — raw immutable snapshots, curated wiki projections, search indexes, and caches may all legitimately hold derived or duplicated representations, connected to the lineage by hashes and provenance — and that shape is already what Prism implements, not a greenfield design.
...
**Still no host lean is expressed between the two live candidates.** The Prism inventory (Q1) has now been done and removes Candidate B — not on maturity grounds (Prism is tested and in daily production use in its own domain) but because hosting AWCP inside it would build a platform from scratch while creating the exact boundary violation ADR-013 exists to prevent. That removal does not resolve the decision: it does not lift Candidate A, whose countervailing entry (ai-memory has shipped none of AWCP's hardest operational risks — session instrumentation, attention precision, corporate integration, approval semantics) is untouched by anything Prism-side. Candidate A's entry in the table above remains a list of *verified reusable capabilities*, not a host-selection argument. The decision between **A (extend ai-memory)** and **C (new umbrella codebase)** is still open and, when taken, needs: (1) the host, (2) module boundaries, (3) the operational-state schema, (4) a supersession plan for duplicated *concepts* (not a data-migration mandate), and (5) the product-wide model-provider abstraction and degraded-capability matrix (§4) as a Must. See `docs/investigations/prism-ground-truth-inventory.md` §4 for the full Candidate B scoring and §6 for two questions this inventory raised that weren't in the original §8 list (corporate/personal isolation now resolved; OD-05's write half identified as cheaply testable).

---

## 8. Clarifying questions (blocking, in priority order)

1. ~~**Prism ground truth:** What does Prism actually do *today* — Confluence/ADO ingestion? snapshotting? search? wiki generation? What store, stack, and interfaces?~~ **Answered 2026-07-28** — see [`docs/investigations/prism-ground-truth-inventory.md`](prism-ground-truth-inventory.md). It did not shift the lean to Candidate B; it removed B from contention (§7 above). The host decision remains open between Candidates A and C.
