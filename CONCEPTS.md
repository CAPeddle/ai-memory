...
The ledger lives inside the schema it manages, so dropping that schema removes the ledger with it and the teardown is a single statement that leaves nothing behind. The checksum is taken over a migration file's raw bytes, with no normalisation of whitespace, line endings, or comments — which makes an applied migration's content effectively frozen, since any later edit to that file, a comment included, no longer matches what was recorded. A mismatch is read as an unknown state rather than a skippable difference: it aborts the entire migration run before anything is applied, and where the feature owning the schema is enabled, it aborts startup, so the process exits rather than serving in a degraded state. Correcting such a file therefore means updating the recorded checksum on every database that holds a row for it, which is an operational change rather than an edit — but that operation only rewrites the record of the bytes, not the schema itself. A version already present in the ledger is skipped rather than re-run, so the rewrite never replays the file's statements: a database that already applied the migration keeps whatever the original bytes produced, while a database migrating from scratch reads the edited file and produces something else. That makes a checksum rewrite sound only for a byte change provably inert in behaviour, such as a comment or a whitespace normalisation. Correcting what a migration actually does requires a new forward migration instead — there is no checksum-only path that brings an already-applied database in line with a corrected statement.
