...
**The stated mechanism above is incomplete, and the gap is load-bearing.** "Harvests co-authors from the commits being squashed" implies the branch is where you check, so a clean branch reads as a guarantee. It is not one. PR #53 (merged as `80befe3`) had **zero** `Co-authored-by:` trailers on all seven of its commits — verified with git's own trailer parser before the merge, not by grep — and GitHub appended the separator plus `Co-authored-by: Christopher  Peddle <cpeddle@bimcollab.com>` anyway. That address appears on no commit in the branch. The block can therefore originate in the merge itself rather than in the commits, and no pre-merge inspection rules it out.

Nothing broke on #53 because it deliberately carried no `Story:` trailer. On a board-tracked PR the same append would have silently pushed `Story:` out of trailer scope — the exact failure Rule 2 exists to prevent, arriving by a route Rule 2 does not name.

**So treat the pre-merge check as necessary but not sufficient, and make the post-merge check mandatory.** Verify with `git log -1 --format='%(trailers:key=Story,valueonly)'` on the merged commit — it returns empty when either rule was broken, and `--grep` alone will not tell you, because the trailer *text* survives in the body either way. When it comes back empty on a board-tracked squash, the fix is `git commit --amend` on the merge commit (or a follow-up commit carrying the trailer) before anything builds on it.
