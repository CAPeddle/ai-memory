...
**Story board:** [.github/planning/story-board.md](../.github/planning/story-board.md) — **ST-088 is under `## Done` as of 2026-08-27.** That entry is authoritative for WIP limits and acceptance criteria. A session resuming this milestone will find neither an In Progress nor a Review section to route to — **both WIP slots are free** — because the milestone is **finished**: Phase 4 closed, ADR-016 Accepted with Candidate A **rejected**, and the sign-off PR ([#60](https://github.com/CAPeddle/ai-memory/pull/60)) merged as `86473ac`. *(This line said "under `## Review` … waits only on its sign-off PR merging" until 2026-08-27; that merge has happened.)* **Do not resume delivery work here** — follow-on is **ST-100** (score the peer-service topology) and **ST-082** (policy-scope enforcement, ai-memory's own obligation), both in Backlog.
